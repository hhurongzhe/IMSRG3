...
## ReadOmegas.py
##
## Transform operatos by reading HF (or NAT) transformation coefficients 
## and IMSRG Omegas from a previous calculation of magnus method.
##                                              --Baishan Hu
##                                              TRIUMF Oct 2022
######################################################################

import sys, os
#sys.path.append('/global/home/bhu/lib/')
sys.path.append('/home/bhu/lib/')
#sys.path.append('/ccs/home/baishan/lib/')
# import numpy as np
import math
from pyIMSRG import *
...
def main_job(args_v):
    ...
    emax=0; e2max=0; e3max = 0
    emax_imsrg = -1; e2max_imsrg = -1; e3max_imsrg = 0
    ...
    for args in args_v:
        arr=args.split('=')
        key = arr[0].strip(); value = arr[1].strip()
        print(key, '=>', value)

        if(key=='2bme'): inputtbme = value
        if(key=='3bme'): input3bme = value
        if(key=='3bme_type'): input3bme_type = value
        if(key=='no2b_precision'): no2b_precision = value
        if(key=='reference'): reference = value
        if(key=='valence_space'): name_vs = value
        if(key=='custom_valence_space'): custom_valence_space = value
        if(key=='basis'): basis = value
        if(key=='method'): method = value
        if(key=='flowfile'): flowfile = value
        if(key=='intfile'): intfile = value
        if(key=='core_generator'): core_generator = value
        if(key=='valence_generator'): valence_generator = value
        if(key=='fmt2'): fmt2 = value
        if(key=='fmt3'): fmt3 = value
        if(key=='input_op_fmt'): input_op_fmt = value
        if(key=='denominator_delta_orbit'): denominator_delta_orbit = value
        if(key=='LECs'): LECs = value
        if(key=='scratch'): scratch = value
        if(key=='valence_file_format'): valence_file_format = value
        if(key=='occ_file'): occ_file = value
        if(key=='physical_system'): physical_system = value
        if(key=='denominator_partitioning'): denominator_partitioning = value
        if(key=='NAT_order'): NAT_order = value

        if(key=='use_brueckner_bch'): use_brueckner_bch = (value == 'true')
        if(key=='nucleon_mass_correction'): nucleon_mass_correction = (value == 'true')
        if(key=='relativistic_correction'): relativistic_correction = (value == 'true')
        if(key=='IMSRG3'): IMSRG3 = (value == 'true')
        if(key=='imsrg3_n7'): imsrg3_n7 = (value == 'true')
        if(key=='imsrg3_mp4'): imsrg3_mp4 = (value == 'true')
        if(key=='imsrg3_at_end'): imsrg3_at_end = (value == 'true')
        if(key=='imsrg3_no_qqq'): imsrg3_no_qqq = (value == 'true')
        if(key=='write_omega'): write_omega = (value == 'true')
        if(key=='freeze_occupations'): freeze_occupations = (value == 'true')
        if(key=='discard_no2b_from_3n'): discard_no2b_from_3n = (value == 'true')
        if(key=='hunter_gatherer'): hunter_gatherer = (value == 'true')
        if(key=='goose_tank'): goose_tank = (value == 'true')
        if(key=='discard_residual_input3N'): discard_residual_input3N = (value == 'true')
        if(key=='use_NAT_occupations'): use_NAT_occupations = (value == 'true')
        if(key=='order_NAT_by_energy'): order_NAT_by_energy = (value == 'true')
        if(key=='store_3bme_pn'): store_3bme_pn = (value == 'true')
        if(key=='only_2b_eta'): only_2b_eta = (value == 'true')
        if(key=='only_2b_omega'): only_2b_omega = (value == 'true')
        if(key=='perturbative_triples'): perturbative_triples = (value == 'true')
        if(key=='write_HO_ops'): write_HO_ops = (value == 'true')
        if(key=='write_HF_ops'): write_HF_ops = (value == 'true')

        if(key=='emax'): emax = int(value)
        if(key=='lmax'): lmax = int(value)
        if(key=='e3max'): e3max = int(value)
        if(key=='lmax3'): lmax3 = int(value)
        if(key=='A'): targetMass = int(value)
        if(key=='nsteps'): nsteps = int(value)
        if(key=='file2e1max'): file2e1max = int(value)
        if(key=='file2e2max'): file2e2max = int(value)
        if(key=='file2lmax'): file2lmax = int(value)
        if(key=='file3e1max'): file3e1max = int(value)
        if(key=='file3e2max'): file3e2max = int(value)
        if(key=='file3e3max'): file3e3max = int(value)
        if(key=='atomicZ'): atomicZ = int(value)
        if(key=='emax_unocc'): emax_unocc = int(value)
        if(key=='emax_imsrg'): emax_imsrg = int(value)
        if(key=='e2max_imsrg'): e2max_imsrg = int(value)
        if(key=='e3max_imsrg'): e3max_imsrg = int(value)
        if(key=='emax_3body_imsrg'): emax_3body_imsrg = int(value)

        if(key=='hw'): hw = float(value)
        if(key=='smax'):  smax = int(value)
        if(key=='ode_tolerance'): ode_tolerance = float(value)
        if(key=='dsmax'):  dsmax = float(value)
        if(key=='ds_0'): ds_0 = float(value)
        if(key=='domega'): domega = float(value)
        if(key=='omega_norm_max'):  omega_norm_max = float(value)
        if(key=='denominator_delta'): denominator_delta = float(value)
        if(key=='BetaCM'): BetaCM = float(value)
        if(key=='hwBetaCM'): hwBetaCM = float(value)
        if(key=='eta_criterion'): eta_criterion = float(value)
        if(key=='hw_trap'): hw_trap = float(value)
        if(key=='dE3max'): dE3max = float(value)
        if(key=='OccNat3Cut'): OccNat3Cut = float(value)
        if(key=='threebody_threshold'): threebody_threshold = float(value)

        if(key=='Z'): Z = int(value)
        if(key=='e2max'): e2max = int(value)
        if(key=='jobname'): jobname = value
        if(key=='path_omega'): path_omega = value
        if(key=='path_output'): path_output = value

        if(key=='smax_core'):  smax_core = int(value)
        if(key=='smax_valence'):  smax_valence = int(value)
        if(key=='approx'): approx = value

        if(key=='Operators'): ops = value.split(',')
        if(key=='OperatorsFromFile'): opsfromfile = value.split(',')

    if(emax_imsrg == -1 or e2max_imsrg == -1):
        emax_imsrg = emax; e2max_imsrg = emax*2; e3max_imsrg = e3max

    print('\n---------------')
    if( find('jobname',args_v) ):
    	print('Working with:', jobname)
    if(ops != ''): print('Working for Operator:', ops)
    if(opsfromfile != ''): print('Working for Operator from file::', opsfromfile)


    rw = ReadWrite()
    #if( find('LECs',args_v) ):
    #    rw.SetLECs_preset(LECs)
    if( 'scratch' in args_v ):
    ...
    if(custom_valence_space != ''): valence_space = custom_valence_space
    ...
    if(inputtbme != 'none'):
        if(fmt2 == 'me2j'):
            ...
            if(LECs == 'Minnesota'): Hbare += OperatorFromString(ms,'VMinnesota')
    ...
    if( find('BetaCM',args_v) and abs(BetaCM)>1e-6):
        if(hwBetaCM < 0): hwBetaCM = ms.GetHbarOmega()
    ...
    particle_rank = 2
    if approx=='imsrg3n7': particle_rank = 3
    ...
    for op_temp in opall:
        ...
        if(op_temp in ops):
            ...
            opff = op_temp.split('^')
            opname = opff[0]; qnumbers = opff[1]; f2name = opff[2]
            name_op = opname
            if(len(opff)>3): f3name = opff[3]
            q_temp = qnumbers.split('_')
            op_j=int(q_temp[0]); op_t=int(q_temp[1]); op_p=int(q_temp[2]); op_r=int(q_temp[3])
            OpHO =  Operator(ms,op_j,op_t,op_p,op_r)

            if(op_r>2): OpHO.ThreeBody.Allocate()
        ...
        if( find('path_omega',args_v) ):
            ...
            while True:
                ...
                if(os.path.exists(name_omega)):
                    ...
                    Op = Commutator.BCH_Transform(Op, Omega)
