...
double jho(int na0, int la, int nb0, int lb, int L, double y)
{
    ...
    for (int k = 0; k < nb; k++)
    {
        ...
        {
            f3 += pow(-1, k1) / gsl_sf_fact(k1) / gsl_sf_fact(na - 1 - k1) * gsl_sf_gamma(0.5 * (L + lb + la + 2 * k + 2 * k1 + 3.0)) / gsl_sf_gamma(la + k1 + 1.5) * gsl_sf_hyperg_1F1(0.5 * (L - lb - la - 2 * k - 2 * k1), L + 1.5, y);
        }
        j_ho += f2 * f3;
    }
    j_ho *= f1;
...
double jdmho(int na0, int la, int nb0, int lb, int L, double y)
{
    ...
    double f1 = pow(2, L - 1) / gsl_sf_doublefact(2 * L + 1) * pow(y, 0.5 * (L - 1)) * exp(-y) * sqrt(gsl_sf_fact(nb - 1) * gsl_sf_fact(na - 1) * gsl_sf_gamma(na + la + 0.5) * gsl_sf_gamma(nb + lb + 0.5));
    ...
    for (int k = 0; k < nb; k++)
    {
        ...
        for (int k1 = 0; k1 < na; k1++)
        {
            f3 += pow(-1, k1) / gsl_sf_fact(k1) / gsl_sf_fact(na - 1 - k1) * gsl_sf_gamma(0.5 * (L + lb + la + 2 * k + 2 * k1 + 2.0)) / gsl_sf_gamma(la + k1 + 1.5) *
                  (-(L + lb + la + 2 * k + 2 * k1 + 2) * 0.5 * gsl_sf_hyperg_1F1(0.5 * (L - lb - la - 2 * k - 2 * k1 - 1), L + 1.5, y) + (2 * k) * gsl_sf_hyperg_1F1(0.5 * (L - lb - la - 2 * k - 2 * k1 + 1), L + 1.5, y) // different from two lines above by -1 -> +1.
                  );
        }
        j_dmho += f2 * f3;
    }
...
double jdpho(int na0, int la, int nb0, int lb, int L, double y)
{
    ...
    for (int k = 0; k < nb; k++)
    {
        ...
        {
            f3 += pow(-1, k1) / gsl_sf_fact(k1) / gsl_sf_fact(na - 1 - k1) * gsl_sf_gamma(0.5 * (L + lb + la + 2 * k + 2 * k1 + 2.0)) / gsl_sf_gamma(la + k1 + 1.5) *
                  (-(L + lb + la + 2 * k + 2 * k1 + 2) * 0.5 * gsl_sf_hyperg_1F1(0.5 * (L - lb - la - 2 * k - 2 * k1 - 1), L + 1.5, y) + (2 * lb + 2 * k + 1) * gsl_sf_hyperg_1F1(0.5 * (L - lb - la - 2 * k - 2 * k1 + 1), L + 1.5, y) // different from two lines above by -1 -> +1.
                  );
        }
        j_dpho += f2 * f3;
    }
...
Operator M(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : M_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double mab = 1.0 / sqrt(4 * M_PI) * pow(-1, (2 * J + j2b + 1) * 0.5) * sqrt((2 * la + 1.) * (2 * lb + 1.) * (j2a + 1.) * (j2b + 1.) * (2 * J + 1.)) * gsl_sf_coupling_6j(2 * la, j2a, 1, j2b, 2 * lb, 2 * J) *
...
Operator Ms(ModelSpace &modelspace, std::string IsoSV, int J, int L, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : Ms_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double mab = sqrt(1.5 / M_PI) * pow(-1, la) * sqrt((2 * la + 1.) * (2 * lb + 1.) * (j2a + 1.) * (j2b + 1.) * (2 * L + 1) * (2 * J + 1.)) * gsl_sf_coupling_9j(2 * la, 2 * lb, 2 * L, 1, 1, 2, j2a, j2b, 2 * J) * gsl_sf_coupling_3j(2 * la, 2 * L, 2 * lb, 0, 0, 0) * jho(na, la, nb, lb, L, y);
            // This formula assumes we're storing things as reduced matrix elements. If J=0, then we aren't, so we get a factor sqrt(2j+1)
            if (J == 0 and parity == 0)
            {
                mab /= sqrt(j2a + 1.0);
...
Operator Mg(ModelSpace &modelspace, std::string IsoSV, int J, int L, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : Mg_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double mab = 1.0 / sqrt(4.0 * M_PI) * pow(-1, (2 * L + j2b + 1.0) * 0.5) * sqrt((2 * la + 1) * (j2a + 1) * (j2b + 1) * (2 * L + 1) * (2 * J + 1)) * gsl_sf_coupling_6j(2 * la, j2a, 1, j2b, 2 * lb, 2 * J) *
                         (-sqrt(lb + 1.0) * sqrt(2 * (lb + 1) + 1) * gsl_sf_coupling_6j(2 * L, 2, 2 * J, 2 * lb, 2 * la, 2 * (lb + 1)) * gsl_sf_coupling_3j(2 * la, 2 * L, 2 * (lb + 1), 0, 0, 0) * jdmho(na, la, nb, lb, L, y) + sqrt(lb) * sqrt(2 * (lb - 1) + 1) * gsl_sf_coupling_6j(2 * L, 2, 2 * J, 2 * lb, 2 * la, 2 * (lb - 1)) * gsl_sf_coupling_3j(2 * la, 2 * L, 2 * (lb - 1), 0, 0, 0) * jdpho(na, la, nb, lb, L, y));
            // This formula assumes we're storing things as reduced matrix elements. If J=0, then we aren't, so we get a factor sqrt(2j+1)
            if (J == 0 and parity == 0)
            {
                mab /= sqrt(j2a + 1.0);
...
Operator Sigmap(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    if (J > 0)
        Sigmap_op += sqrt((J + 1.) / (2 * J + 1.)) * Ms(modelspace, IsoSV, J, J - 1, q);
    return Sigmap_op;
}
...
Operator Delta(ModelSpace &modelspace, std::string IsoSV, int J, double q) { return Mg(modelspace, IsoSV, J, J, q); }
...
Operator Deltap(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    if (J > 0)
        Deltap_op += sqrt((J + 1.) / (2 * J + 1.)) * Mg(modelspace, IsoSV, J, J - 1, q);
...
double PhiF(int la, int j2a, int j2b) { return pow(-1, la + 1) * 6 * sqrt((2 * la + 1) * (j2a + 1) * (j2b + 1)) / sqrt(4 * M_PI); }
...
double PhiS1(int na, int la, int j2a, int nb, int lb, int j2b, int J, double y)
{
    ...
    {
        phis1 += pow(-1, J - lam + 1) * sqrt((2 * lam + 1) * (2 * lam + 1)) * gsl_sf_coupling_6j(2 * (J + 1), 2, 2 * lam, 2, 2 * J, 2) * gsl_sf_coupling_6j(2 * (J + 1), 2, 2 * lam, 2 * lb, 2 * la, 2 * (lb + 1)) * gsl_sf_coupling_9j(2 * la, 2 * lb, 2 * lam, 1, 1, 2, j2a, j2b, 2 * J);
    }

    phis1 *= sqrt(2 * (lb + 1) + 1) * sqrt(lb + 1) * gsl_sf_coupling_3j(2 * la, 2 * (J + 1), 2 * (lb + 1), 0, 0, 0) * jdmho(na, la, nb, lb, J + 1, y);
...
double PhiS2(int na, int la, int j2a, int nb, int lb, int j2b, int J, double y)
{
    ...
    for (int lam = J; lam <= J + 1; lam++)
    {
        phis2 += pow(-1, J - lam) * sqrt((2 * lam + 1) * (2 * lam + 1)) * gsl_sf_coupling_6j(2 * (J + 1), 2, 2 * lam, 2, 2 * J, 2) * gsl_sf_coupling_6j(2 * (J + 1), 2, 2 * lam, 2 * lb, 2 * la, 2 * (lb - 1)) * gsl_sf_coupling_9j(2 * la, 2 * lb, 2 * lam, 1, 1, 2, j2a, j2b, 2 * J);
    }

    phis2 *= sqrt(2 * (lb - 1) + 1) * sqrt(lb) * gsl_sf_coupling_3j(2 * la, 2 * (J + 1), 2 * (lb - 1), 0, 0, 0) * jdpho(na, la, nb, lb, J + 1, y);
...
double PhiS3(int na, int la, int j2a, int nb, int lb, int j2b, int J, double y)
{
    ...
    {
        phis3 += pow(-1, J - lam + 1) * sqrt((2 * lam + 1) * (2 * lam + 1)) * gsl_sf_coupling_6j(2 * (J - 1), 2, 2 * lam, 2, 2 * J, 2) * gsl_sf_coupling_6j(2 * (J - 1), 2, 2 * lam, 2 * lb, 2 * la, 2 * (lb + 1)) * gsl_sf_coupling_9j(2 * la, 2 * lb, 2 * lam, 1, 1, 2, j2a, j2b, 2 * J);
    }

    phis3 *= sqrt(2 * (lb + 1) + 1) * sqrt(lb + 1) * gsl_sf_coupling_3j(2 * la, 2 * (J - 1), 2 * (lb + 1), 0, 0, 0) * jdmho(na, la, nb, lb, J - 1, y);
...
double PhiS4(int na, int la, int j2a, int nb, int lb, int j2b, int J, double y)
{
    ...
    for (int lam = J - 1; lam <= J; lam++)
    {
        phis4 += pow(-1, J - lam) * sqrt((2 * lam + 1) * (2 * lam + 1)) * gsl_sf_coupling_6j(2 * (J - 1), 2, 2 * lam, 2, 2 * J, 2) * gsl_sf_coupling_6j(2 * (J - 1), 2, 2 * lam, 2 * lb, 2 * la, 2 * (lb - 1)) * gsl_sf_coupling_9j(2 * la, 2 * lb, 2 * lam, 1, 1, 2, j2a, j2b, 2 * J);
    }

    phis4 *= sqrt(2 * (lb - 1) + 1) * sqrt(lb) * gsl_sf_coupling_3j(2 * la, 2 * (J - 1), 2 * (lb - 1), 0, 0, 0) * jdpho(na, la, nb, lb, J - 1, y);
...
Operator Phip(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : Phip_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double phip_ab = PhiF(la, j2a, j2b) * (-sqrt(2 * (J + 1) + 1) * sqrt(J) * (PhiS1(na, la, j2a, nb, lb, j2b, J, y) + PhiS2(na, la, j2a, nb, lb, j2b, J, y)));

            if (J > 0)
                phip_ab += PhiF(la, j2a, j2b) * (sqrt(2 * (J - 1) + 1) * sqrt(J + 1) * (PhiS3(na, la, j2a, nb, lb, j2b, J, y) + PhiS4(na, la, j2a, nb, lb, j2b, J, y)));
...
Operator Phipp(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : Phipp_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double phipp_ab = PhiF(la, j2a, j2b) * (sqrt(2 * (J + 1) + 1) * sqrt(J + 1) * (PhiS1(na, la, j2a, nb, lb, j2b, J, y) + PhiS2(na, la, j2a, nb, lb, j2b, J, y)));

            if (J > 0)
                phipp_ab += PhiF(la, j2a, j2b) * (sqrt(2 * (J - 1) + 1) * sqrt(J) * (PhiS3(na, la, j2a, nb, lb, j2b, J, y) + PhiS4(na, la, j2a, nb, lb, j2b, J, y)));
...
Operator Omega(ModelSpace &modelspace, std::string IsoSV, int J, double q)
{
    ...
    for (int a = 0; a < norb; a++)
    {
        ...
        for (int b : Omega_op.OneBodyChannels.at({la, j2a, oa.tz2}))
        {
            ...
            double omega_ab = 1.0 / sqrt(4 * M_PI) * modelspace.phase(la) * sqrt((2 * la + 1) * (j2a + 1) * (j2b + 1) * (2 * (j2b - lb) + 1) * (2 * J + 1)) * gsl_sf_coupling_6j(2 * la, j2a, 1, j2b, 2 * (j2b - lb), 2 * J) * gsl_sf_coupling_3j(2 * la, 2 * J, 2 * (j2b - lb), 0, 0, 0) * (O1 + O2);
