...
ThreeBodyME::ThreeBodyME() : threebody_storage(new ThreeBodyStorage_iso()), modelspace(NULL), E3max(0), herm(1) {}

ThreeBodyME::ThreeBodyME(ModelSpace *ms) : threebody_storage(new ThreeBodyStorage_iso()), modelspace(ms), E3max(ms->E3max), emax(ms->GetEMax3Body()), herm(1) {}

ThreeBodyME::ThreeBodyME(ModelSpace *ms, int rJ, int rT, int p) : threebody_storage(new ThreeBodyStorage_iso(ms, rJ, rT, p)), modelspace(ms), E3max(ms->E3max), emax(ms->GetEMax3Body()), herm(1), rank_J(rJ), rank_T(rT), parity(p)
{
...
ThreeBodyME::ThreeBodyME(ModelSpace *ms, int e3max) : threebody_storage(new ThreeBodyStorage_iso(ms, e3max)), modelspace(ms), E3max(e3max), emax(ms->GetEMax3Body()), herm(1) {}

ThreeBodyME::ThreeBodyME(ModelSpace *ms, int e3max, int rJ, int rT, int p) : threebody_storage(new ThreeBodyStorage_iso(ms, e3max, rJ, rT, p)), modelspace(ms), E3max(e3max), emax(ms->GetEMax3Body()), herm(1), rank_J(rJ), rank_T(rT), parity(p)
{
    ISOSPIN_BLOCK_DIMENSION = 5;
    if (rank_T == 1)
        ISOSPIN_BLOCK_DIMENSION = 9;
...
ThreeBodyME::ThreeBodyME(const ThreeBodyME &Tbme)
    : threebody_storage(Tbme.threebody_storage->Clone()), modelspace(Tbme.modelspace),
      ...
      E3max(Tbme.E3max), emax(Tbme.emax), herm(Tbme.herm), rank_J(Tbme.rank_J), rank_T(Tbme.rank_T), parity(Tbme.parity)
{
...
/// \f]
//*******************************************************************
ThreeBME_type ThreeBodyME::GetME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b, int c, int d, int e, int f) const { return threebody_storage->GetME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f); }

ThreeBME_type ThreeBodyME::GetME_pn_ch(size_t ch_bra, size_t ch_ket, size_t ibra, size_t iket) const { return threebody_storage->GetME_pn_ch(ch_bra, ch_ket, ibra, iket); }

// Get a 3-body matrix element in isospin formalism, regardless of how it is stored.
ThreeBME_type ThreeBodyME::GetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a, int b, int c, int d, int e, int f) const { return threebody_storage->GetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoTabc, twoTdef, a, b, c, d, e, f); }
...
ThreeBME_type ThreeBodyME::GetME_pn(int Jab_in, int j0, int Jde_in, int j1, int a, int b, int c, int d, int e, int f) const { return threebody_storage->GetME_pn(Jab_in, j0, Jde_in, j1, a, b, c, d, e, f); }

void ThreeBodyME::SetME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b, int c, int d, int e, int f, ThreeBME_type V) { threebody_storage->SetME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f, V); }

void ThreeBodyME::SetME_pn_ch(size_t ch_bra, size_t ch_ket, size_t ibra, size_t iket, ThreeBME_type V) { threebody_storage->SetME_pn_ch(ch_bra, ch_ket, ibra, iket, V); }

void ThreeBodyME::SetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a, int b, int c, int d, int e, int f, ThreeBME_type V) { threebody_storage->SetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoTabc, twoTdef, a, b, c, d, e, f, V); }
...
void ThreeBodyME::AddToME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a, int b, int c, int d, int e, int f, ThreeBME_type V) { threebody_storage->AddToME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoTabc, twoTdef, a, b, c, d, e, f, V); }

void ThreeBodyME::AddToME_pn_ch(size_t ch_bra, size_t ch_ket, size_t ibra, size_t iket, ThreeBME_type V) { threebody_storage->AddToME_pn_ch(ch_bra, ch_ket, ibra, iket, V); }
...
void ThreeBodyME::AddToME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b, int c, int d, int e, int f, ThreeBME_type V) { threebody_storage->AddToME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f, V); }

std::vector<double> ThreeBodyME::GetME_pn_TwoOps(int Jab, int Jde, int twoJ, int a, int b, int c, int d, int e, int f, const ThreeBodyME &X, const ThreeBodyME &Y) const { return threebody_storage->GetME_pn_TwoOps(Jab, Jde, twoJ, a, b, c, d, e, f, *(X.threebody_storage), *(Y.threebody_storage)); }

ThreeBME_type ThreeBodyME::GetME_pn_no2b(int a, int b, int c, int d, int e, int f, int J2b) const { return threebody_storage->GetME_pn_no2b(a, b, c, d, e, f, J2b); }

ThreeBME_type ThreeBodyME::GetME_pn_mono(int a, int b, int c, int d, int e, int f) const { return threebody_storage->GetME_pn_mono(a, b, c, d, e, f); }
    ...
    for (size_t ich = 0; ich < nch; ich++)
    {
        ...
        for (size_t ibra = 0; ibra < nbras; ibra++)
        {
            ...
            for (size_t iket = iketmin; iket < nkets; iket++)
            {
                ...
                if ((ch_bra == ch_ket) and (ibra == iket) and (herm == -1))
                    continue;
...
bool ThreeBodyME::IsKetValid(int Jab_in, int twoJ, size_t a_in, size_t b_in, size_t c_in) const { return threebody_storage->IsKetValid(Jab_in, twoJ, a_in, b_in, c_in); }

bool ThreeBodyME::IsKetInEMaxTruncations(size_t a_in, size_t b_in, size_t c_in) const { return threebody_storage->IsKetInEMaxTruncations(a_in, b_in, c_in); }

bool ThreeBodyME::IsOrbitIn3BodyEMaxTruncation(size_t a) const { return threebody_storage->IsOrbitIn3BodyEMaxTruncation(a); }

bool ThreeBodyME::IsOrbitIn3BodyEMaxTruncation(const Orbit &oa) const { return threebody_storage->IsOrbitIn3BodyEMaxTruncation(oa); }

size_t ThreeBodyME::GetKetIndex_withRecoupling(int Jab_in, int twoJ, size_t a_in, size_t b_in, size_t c_in, std::vector<size_t> &iket, std::vector<double> &recouple) const { return threebody_storage->GetKetIndex_withRecoupling(Jab_in, twoJ, a_in, b_in, c_in, iket, recouple); }
...
//*******************************************************************
double ThreeBodyME::RecouplingCoefficient(ThreeBodyStorage::Permutation recoupling_case, double ja, double jb, double jc, int Jab_in, int Jab, int twoJ) const { return threebody_storage->RecouplingCoefficient(recoupling_case, ja, jb, jc, Jab_in, Jab, twoJ); }

int ThreeBodyME::PermutationPhase(ThreeBodyStorage::Permutation recoupling_case) const { return threebody_storage->PermutationPhase(recoupling_case); }

void ThreeBodyME::Permute(ThreeBodyStorage::Permutation perm, size_t a_in, size_t b_in, size_t c_in, size_t &a_out, size_t &b_out, size_t &c_out) { threebody_storage->Permute(perm, a_in, b_in, c_in, a_out, b_out, c_out); }

std::vector<ThreeBodyStorage::Permutation> ThreeBodyME::UniquePermutations(size_t a, size_t b, size_t c) const { return threebody_storage->UniquePermutations(a, b, c); }

std::unordered_map<ThreeBodyStorageChannel, size_t, ThreeBodyStorageChannelHash> &ThreeBodyME::Get_ch_start() const { return threebody_storage->ch_start; }

std::vector<size_t> &ThreeBodyME::Get_ch_dim() { return threebody_storage->ch_dim; }
...
void ThreeBodyME::WriteBinary(std::ofstream &f) { threebody_storage->WriteBinary(f); }

void ThreeBodyME::ReadBinary(std::ifstream &f) { threebody_storage->ReadBinary(f); }

void ThreeBodyME::WriteFile(std::vector<std::string> StringInputs, std::vector<int> IntInputs) const { threebody_storage->WriteFile(StringInputs, IntInputs); }

void ThreeBodyME::ReadFile(std::vector<std::string> StringInputs, std::vector<int> IntInputs) { threebody_storage->ReadFile(StringInputs, IntInputs); }

std::string ThreeBodyME::GetStorageMode() const { return threebody_storage->GetStorageMode(); }
