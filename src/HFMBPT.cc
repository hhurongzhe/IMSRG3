...
HFMBPT::HFMBPT(Operator &hbare) : HartreeFock(hbare), C_HO2NAT(modelspace->GetNumberOrbits(), modelspace->GetNumberOrbits(), arma::fill::eye), C_HF2NAT(modelspace->GetNumberOrbits(), modelspace->GetNumberOrbits(), arma::fill::eye), use_NAT_occupations(false), NAT_order("occupation") {}
    ...
    if (use_NAT_occupations) // use fractional occupation
    {
        ...
        while ((Z - ZfromTr) > ModelSpace::OCC_CUT)
        {
            for (size_t i = 0; i < holeorbs_tmp.size(); i++)
            {
                ...
                if ((Z - ZfromTr) < ModelSpace::OCC_CUT)
                    break;
            }
        }
        ...
        while ((N - NfromTr) > ModelSpace::OCC_CUT)
        {
            for (size_t i = 0; i < holeorbs_tmp.size(); i++)
            {
                ...
                if ((N - NfromTr) < ModelSpace::OCC_CUT)
                    break;
            }
        }
    ...
    for (auto &it : OpHF.TwoBody.MatEl)
    {
        ...
        for (int i = 0; i < nkets; ++i)
        {
            ...
            for (int j = 0; j < nkets; ++j)
            {
                ...
                {
                    Dket(i, j) += C_HF2NAT(ket_hf.q, ket_nat.p) * C_HF2NAT(ket_hf.p, ket_nat.q) * ket_hf.Phase(tbc_ket.J);
                }
                if (ket_hf.p == ket_hf.q)
                    Dket(i, j) *= PhysConst::SQRT2;
                if (ket_nat.p == ket_nat.q)
                    Dket(i, j) /= PhysConst::SQRT2;
            }
        }
        if (ch_bra == ch_ket)
        ...
        else
        {
            for (int i = 0; i < nbras; ++i)
            {
                ...
                for (int j = 0; j < nbras; ++j)
                {
                    ...
                    {
                        Dbra(i, j) += C_HF2NAT(bra_hf.q, bra_nat.p) * C_HF2NAT(bra_hf.p, bra_nat.q) * bra_hf.Phase(tbc_bra.J);
                    }
                    if (bra_hf.p == bra_hf.q)
                        Dbra(i, j) *= PhysConst::SQRT2;
                    if (bra_nat.p == bra_nat.q)
                        Dbra(i, j) /= PhysConst::SQRT2;
                }
            }
        }
    ...
    for (auto &it : OpHO.TwoBody.MatEl)
    {
        ...
        for (int i = 0; i < nkets; ++i)
        {
            ...
            for (int j = 0; j < nkets; ++j)
            {
                ...
                {
                    Dket(i, j) += C_HO2NAT(ket_ho.q, ket_nat.p) * C_HO2NAT(ket_ho.p, ket_nat.q) * ket_ho.Phase(tbc_ket.J);
                }
                if (ket_ho.p == ket_ho.q)
                    Dket(i, j) *= PhysConst::SQRT2;
                if (ket_nat.p == ket_nat.q)
        ...
        else
        {
            for (int i = 0; i < nbras; ++i)
            {
                ...
                for (int j = 0; j < nbras; ++j)
                {
                    ...
                    {
                        Dbra(i, j) += C_HO2NAT(bra_ho.q, bra_nat.p) * C_HO2NAT(bra_ho.p, bra_nat.q) * bra_ho.Phase(tbc_bra.J);
                    }
                    if (bra_ho.p == bra_ho.q)
                        Dbra(i, j) *= PhysConst::SQRT2;
                    if (bra_nat.p == bra_nat.q)
...
void HFMBPT::GetDensityMatrix()
{
    ...
    for (auto &i : HartreeFock::modelspace->holes)
        rho(i, i) = HartreeFock::modelspace->GetOrbit(i).occ; // Set hole occupations to 1.
...
void HFMBPT::PrintOccupation()
{
    ...
    {
        Orbit &oi = HartreeFock::modelspace->GetOrbit(i);
        std::cout << std::fixed << std::setw(4) << oi.n << std::setw(4) << oi.l << std::setw(4) << oi.j2 << std::setw(4) << oi.tz2 << "   " << std::setw(8) << Occ(i) << std::endl;
    }
}
...
void HFMBPT::DensityMatrixPP(Operator &H)
{
    ...
    for (auto &a : HartreeFock::modelspace->particles)
    {
        ...
        if ((1 - oa.occ) < ModelSpace::OCC_CUT)
            continue;
        ...
        for (auto &b : modelspace->OneBodyChannels.at({oa.l, oa.j2, oa.tz2}))
        {
            ...
            Orbit &ob = HartreeFock::modelspace->GetOrbit(b);
            if ((1 - ob.occ) < ModelSpace::OCC_CUT)
                continue;
            ...
            //      for(auto& c : HartreeFock::modelspace->all_orbits)
            {
                ...
                if ((1 - oc.occ) < ModelSpace::OCC_CUT)
                    continue;
                ...
                for (auto &i : HartreeFock::modelspace->holes)
                {
                    ...
                    for (auto &j : HartreeFock::modelspace->holes)
                    {
                        ...
                        if (oj.occ < ModelSpace::OCC_CUT)
                            continue;

                        double e_acij = ea + ec - ei - ej;
                        double e_bcij = eb + ec - ei - ej;
                        if (std::abs(e_acij * e_bcij) < 1.e-8)
                            continue;
                        ...
                        {
                            tbme += (2 * J + 1) * H.TwoBody.GetTBME_J(J, a, c, i, j) * H.TwoBody.GetTBME_J(J, i, j, b, c);
                        }
            ...
            //      for(auto& a : HartreeFock::modelspace->all_orbits)
            {
                ...
                //        for(auto& b : HartreeFock::modelspace->all_orbits)
                {
                    ...
                    for (auto &k : HartreeFock::modelspace->holes)
                    {
                        ...
                        if (ok.occ < ModelSpace::OCC_CUT)
                            continue;

                        double e_abik = ea + eb - ei - ek;
                        double e_abjk = ea + eb - ek - ej;
                        if (std::abs(e_abik * e_abjk) < 1.e-8)
                            continue;
                        ...
                        for (int J = Jmin; J <= Jmax; ++J)
                        {
                            tbme += (2 * J + 1) * H.TwoBody.GetTBME_J(J, a, b, i, k) * H.TwoBody.GetTBME_J(J, j, k, a, b);
                        }
            ...
            for (auto &b : HartreeFock::modelspace->particles)
            {
                ...
                for (auto &c : HartreeFock::modelspace->particles)
                {
                    ...
                    for (auto &j : HartreeFock::modelspace->holes)
                    {
                        ...
                        for (int J = Jmin; J <= Jmax; ++J)
                        {
                            tbme += (2 * J + 1) * H.TwoBody.GetTBME_J(J, a, j, b, c) * H.TwoBody.GetTBME_J(J, b, c, i, j);
                        }
            ...
            for (auto &b : HartreeFock::modelspace->particles)
            {
                ...
                for (auto &j : HartreeFock::modelspace->holes)
                {
                    ...
                    for (auto &k : HartreeFock::modelspace->holes)
                    {
                        ...
                        for (int J = Jmin; J <= Jmax; ++J)
                        {
                            tbme += (2 * J + 1) * H.TwoBody.GetTBME_J(J, k, j, i, b) * H.TwoBody.GetTBME_J(J, a, b, k, j);
                        }
...
void HFMBPT::PrintSPEandWF()
{
    ...
    std::cout << std::fixed << std::setw(3) << "i" << ": " << std::setw(3) << "n" << " " << std::setw(3) << "l" << " " << std::setw(3) << "2j" << " " << std::setw(3) << "2tz" << "   " << std::setw(12) << "SPE" << " " << std::setw(12) << "occ."
              << " " << std::setw(12) << "occNAT" << "   |   " << " overlaps" << std::endl;
    for (auto i : modelspace->all_orbits)
    {
        Orbit &oi = modelspace->GetOrbit(i);
        std::cout << std::fixed << std::setw(3) << i << ": " << std::setw(3) << oi.n << " " << std::setw(3) << oi.l << " " << std::setw(3) << oi.j2 << " " << std::setw(3) << oi.tz2 << "   " << std::setw(12) << std::setprecision(6) << F_natbasis(i, i) << " " << std::setw(12) << oi.occ << " " << std::setw(12) << oi.occ_nat << "   | ";
