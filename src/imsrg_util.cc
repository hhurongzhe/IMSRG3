...
Operator OperatorFromString(ModelSpace &modelspace, std::string opname)
{
    ...
    if (opname == "R2_p1")
        ...
        theop = Rp2_corrected_Op(modelspace, modelspace.GetTargetMass(), modelspace.GetTargetZ());
    else if (opname == "Rn2")
        theop = Rn2_corrected_Op(modelspace, modelspace.GetTargetMass(), modelspace.GetTargetZ());
    else if (opname == "Rm2")
        theop = Rm2_corrected_Op(modelspace, modelspace.GetTargetMass(), modelspace.GetTargetZ());
    else if (opname == "Rm2lab")
        theop = RSquaredOp(modelspace);
    else if (opname == "ISM")
        theop = MultipoleResponseOp(modelspace, 2, 0, 0); // Isoscalar monopole (see PRC97(2018)054306 ) --added by bhu
    else if (opname == "IVM")
        theop = MultipoleResponseOp(modelspace, 2, 0, 1); // Isovector monopole
    else if (opname == "ISQ")
        theop = MultipoleResponseOp(modelspace, 2, 2, 0); // Isoscalar quadrupole
    else if (opname == "IVQ")
        theop = MultipoleResponseOp(modelspace, 2, 2, 1); // Isosvector quadrupole
    else if (opname == "ISO")
        theop = MultipoleResponseOp(modelspace, 3, 3, 0); // Isoscalar octupole
    else if (opname == "IVO")
        theop = MultipoleResponseOp(modelspace, 3, 3, 1); // Isosvector octupole
    else if (opname == "IVD")
        theop = IVDipoleOp(modelspace, 1, 1); // Isovector dipole (see PRC97(2018)054306 ) --added by bhu
    ...
    else if (opname == "TrelMassCorrection")
        theop = Trel_Masscorrection_Op(modelspace);
    else if (opname == "Rso")
    ...
    else if (opname == "LdotS")
        theop = LdotS_Op(modelspace);
    ...
    else if (opnamesplit[0] == "rhocentralp") // central point proton density, averaged over a sphere of radius rmax
    {
        ...
        for (int ipoint = 1; ipoint < npoints; ipoint++)
            theop += (ipoint + 0.5) * (ipoint + 0.5) * DensityAtR(modelspace, dr * (ipoint + 0.5), "proton");
        for (int ipoint = 1; ipoint < npoints; ipoint++)
    ...
    else if (opnamesplit[0] == "rhocentraln") // central point neutron density, averaged over a sphere of radius rmax
    {
        ...
        double norm = 0.5 * 0.5;
        for (int ipoint = 1; ipoint < npoints; ipoint++)
            theop += (ipoint + 0.5) * (ipoint + 0.5) * DensityAtR(modelspace, dr * (ipoint + 0.5), "neutron");
        for (int ipoint = 1; ipoint < npoints; ipoint++)
            norm += (ipoint + 0.5) * (ipoint + 0.5);
        theop /= norm;
    }
    ...
    else if (opnamesplit[0] == "M0nuCT")
    {
        double R0;
        if (opnamesplit.size() < 2)
            std::cout << "ERROR!!!  " << __func__ << "    need to specify a cutoff for M0nuCT" << std::endl;
    ...
    else if (opnamesplit[0] == "DMNREFT") // Dark matter non-relativistic EFT operators: M+, M-, ...
    {
        ...
        std::map<std::string, Operator (*)(ModelSpace &, std::string, int, double)> dmop = {
            {"M", &DM_NREFT::M}, {"Sigma", &DM_NREFT::Sigma}, {"Sigmap", &DM_NREFT::Sigmap}, {"Sigmapp", &DM_NREFT::Sigmapp}, {"Delta", &DM_NREFT::Delta}, {"Deltap", &DM_NREFT::Deltap}, {"Phip", &DM_NREFT::Phip}, {"Phipp", &DM_NREFT::Phipp}, {"Phitp", &DM_NREFT::Phitp}, {"Omega", &DM_NREFT::Omega}, {"Omegat", &DM_NREFT::Omegat},
        };
...
void WriteSPWaveFunctions(std::vector<std::string> &spwf, HartreeFock &hf, std::string intfile)
{
    ...
    for (index_t i = 0; i < spwf.size(); ++i)
    {
        ...
        for (index_t rstep = 0; rstep < R.size(); ++rstep)
            wf_file << std::fixed << std::setw(10) << std::setprecision(7) << R[rstep] << "   " << std::setw(10) << std::setprecision(7) << PSI[rstep] << std::endl;
        wf_file.close();
    }
}
...
Operator TCM_Op(ModelSpace &modelspace)
{
    ...
    int A = modelspace.GetTargetMass();
    if (A < 1)
        std::cout << "DANGER!!! Calling " << __func__ << "  with A=" << A << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1) // In order to make this parallel, need to precompute the Moshinsky brackets.
    for (int ch = 0; ch < nchan; ++ch)
    {
        ...
        for (int ibra = 0; ibra < nkets; ++ibra)
        {
            ...
            if (2 * (oi.n + oj.n) + oi.l + oj.l > E2max)
                continue;
            for (int iket = ibra; iket < nkets; ++iket)
            {
                ...
                if (2 * (ok.n + ol.n) + ok.l + ol.l > E2max)
                    continue;
...
double Calculate_p1p2(ModelSpace &modelspace, Ket &bra, Ket &ket, int J)
{
    ...
    if (std::abs(fab - fcd) > 2 or std::abs(fab - fcd) % 2 > 0)
        return 0;
...
{
    if (Z == 0)
        return 0.0 * KineticEnergy_Op(modelspace);
    return R2CM_Op(modelspace) + (A - 2.0) / (A * Z) * R2_1body_Op(modelspace, "proton") - 4. / (A * Z) * R2_2body_Op(modelspace, "proton") + 1. / Z * RpSpinOrbitCorrection(modelspace);
}
...
Operator Rn2_corrected_Op(ModelSpace &modelspace, int A, int Z)
{
    if (Z == A)
        return 0.0 * KineticEnergy_Op(modelspace);
    return R2CM_Op(modelspace) + (A - 2.0) / (A * (A - Z)) * R2_1body_Op(modelspace, "neutron") - 4. / (A * (A - Z)) * R2_2body_Op(modelspace, "neutron");
}

Operator Rm2_corrected_Op(ModelSpace &modelspace, int A, int Z) { return (1. / A) * RSquaredOp(modelspace) - R2CM_Op(modelspace); }
...
Operator R2_p1_Op(ModelSpace &modelspace) { return R2_1body_Op(modelspace, "proton"); }
...
Operator R2_1body_Op(ModelSpace &modelspace, std::string option)
{
    ...
    if (option == "neutron" or option == "matter")
        orbitlist.insert(modelspace.neutron_orbits.begin(), modelspace.neutron_orbits.end());
    if (option == "proton" or option == "matter")
        orbitlist.insert(modelspace.proton_orbits.begin(), modelspace.proton_orbits.end());
...
Operator R2_p2_Op(ModelSpace &modelspace) { return R2_2body_Op(modelspace, "proton"); }
...
Operator MagneticMultipoleOp(ModelSpace &modelspace, int L) { return MagneticMultipoleOp_pn(modelspace, L, "both"); }
...
Operator MagneticMultipoleOp_pn(ModelSpace &modelspace, int L, std::string pn)
{
    ...
    bool spin = true;
    bool orbit = true;
    if (pn.find("spin") != std::string::npos)
        orbit = false;
    if (pn.find("orbit") != std::string::npos)
        spin = false;
    int norbits = modelspace.GetNumberOrbits();
    for (int i = 0; i < norbits; ++i)
    {
        Orbit &oi = modelspace.GetOrbit(i);
        if (pn == "proton" and oi.tz2 > 0)
            continue;
        if (pn == "neutron" and oi.tz2 < 0)
            continue;
        ...
        for (int j : ML.OneBodyChannels.at({oi.l, oi.j2, oi.tz2}))
        {
            ...
            ML.OneBody(i, j) = modelspace.phase(jj + L - 0.5) * sqrt((2 * ji + 1) * (2 * jj + 1) * (2 * L + 1) / 4. / 3.1415926) * AngMom::ThreeJ(ji, jj, L, 0.5, -0.5, 0) * (L - kappa) * (gl * (1 + kappa / (L + 1.)) - 0.5 * gs) * r2int;
                ...
                for (int Lab = std::max(std::abs(la - lb), std::abs(Jab - 1)); Lab <= std::min(la + lb, Jab + 1); Lab += 1)
                {
                    for (int Lcd = std::max(std::abs(lc - ld), std::abs(Jcd - 1)) + (Lab + L) % 2; Lcd <= std::min(lc + ld, Jcd + 1); Lcd += 2)
                    {
                        for (int S = std::max(std::abs(Jab - Lab), std::abs(Jcd - Lcd)); S <= std::min(1, std::min(Jab + Lab, Jcd + Lcd)); S += 1)
                        {
                            ...
                            if (std::abs(njab) < 1e-8 or std::abs(njcd) < 1e-8)
                                continue;
...
long double TalmiB(int na, int la, int nb, int lb, int p)
{
    ...
    for (int k = kmin; k <= kmax; ++k)
    {
        ...
        B2 += exp(logB1 + lgamma(la + k + 1) - lgamma(k + 1) + lgamma(p - (la - lb) / 2 - k + 1) - lgamma(2 * p - la + lb - 2 * k + 2) - lgamma(2 * la + 2 * k + 2) - lgamma(na - k + 1) - lgamma(nb - p + q + k + 1) - lgamma(p - q - k + 1));
    }
...
long double TalmiB_SingleTerm(int na, int la, int nb, int lb, int p, int K)
{
    ...
    //             / (  gsl_sf_fact(2*la+2*k+1) * gsl_sf_fact(na-k)
    //                * gsl_sf_fact(nb - p + q + k) * gsl_sf_fact(p-q-k) );
    long double B2 = exp(logB1 + lgamma(la + k + 1) - lgamma(k + 1) + lgamma(p - (la - lb) / 2 - k + 1) - lgamma(2 * p - la + lb - 2 * k + 2) - lgamma(2 * la + 2 * k + 2) - lgamma(na - k + 1) - lgamma(nb - p + q + k + 1) - lgamma(p - q - k + 1));
    //   }
...
{
    double c = 1.0;
    for (int x = std::min(a, b) + 1; x <= std::max(a, b); x++)
        c *= x;
    return (a > b) ? c : 1.0 / c;
}
...
Operator AllowedGamowTeller_Op(ModelSpace &modelspace)
{
    ...
    for (int i = 0; i < norbits; ++i)
    {
        ...
        {
            Orbit &oj = modelspace.GetOrbit(j);
            if (oi.n != oj.n or oi.l != oj.l or oi.tz2 == oj.tz2)
                continue;
...
Operator Sigma_Op(ModelSpace &modelspace) { return Sigma_Op_pn(modelspace, "both"); }
...
Operator VCoulomb_Op(ModelSpace &modelspace, int lmax) // default lmax=99999
{
    ...
    for (int ch = 0; ch < nchan; ++ch)
    {
        TwoBodyChannel &tbc = modelspace.GetTwoBodyChannel(ch);
        if (tbc.Tz >= 0)
            continue; // 2-body coulomb only acts in pp channel
        int J = tbc.J;
        int nkets = tbc.GetNumberKets();
        for (int ibra = 0; ibra < nkets; ++ibra)
        {
            ...
            for (int iket = ibra; iket < nkets; ++iket)
            {
                ...
                for (int Lab = std::abs(la - lb); Lab <= la + lb; ++Lab)
                {
                    for (int Sab = 0; Sab <= 1; ++Sab)
                    {
                        ...
                        double njab = AngMom::NormNineJ(la, sa, ja, lb, sb, jb, Lab, Sab, J);
                        if (std::abs(njab) < 1e-7)
                            continue;
                        int Scd = Sab;
                        int Lcd = Lab;
                        double njcd = AngMom::NormNineJ(lc, sc, jc, ld, sd, jd, Lcd, Scd, J);
                        if (std::abs(njcd) < 1e-7)
                            continue;
                        // Next, transform to rel / com coordinates with Moshinsky tranformation
                        for (int N_ab = 0; N_ab <= fab / 2; ++N_ab) // N_ab = CoM n for a,b
                        {
                                            ...
                                            for (int nprel = 0; 2 * nprel <= eket - (2 * Ncm + Lcm); nprel++)
                                            {
                                                ...
                                                for (int Tp = std::abs(Tz); Tp <= 1; Tp++)
                                                {
                                                    ...
                                                    for (int T = std::abs(Tz); T <= 1; T++)
                                                    {
                                                        ...
                                                        if (pn_case == "npnp")
                                                            iso_to_pn_factor = 0.5;
                                                        if (pn_case == "pnpn")
                                                            iso_to_pn_factor = 0.5 * AngMom::phase(T + Tp);
                                                        if (pn_case == "nppn")
                                                            iso_to_pn_factor = 0.5 * AngMom::phase(T + 1);
                                                        if (pn_case == "pnnp")
                                                            iso_to_pn_factor = 0.5 * AngMom::phase(Tp + 1);

                                                        if ((S + Sp) == 1)
                                                            AplusB += iso_to_pn_factor * asymmfactor * isoA(T, Tp, Tz);
                                                        if ((S == 1) and (Sp == 1))
                                                            AplusB += iso_to_pn_factor * asymmfactor * isoB(T, Tp) * sqrt(2); // sqrt(2) missing here
                                                ...
                                                vpt += sqrt((2 * S + 1) * (2 * Sp + 1)) * AplusB * (2 * L + 1) * (2 * Lp + 1) * sixj1 * ninej1 * ninej2 * AngMom::phase(Lcm) * sqrt((2 * lrel + 1) * (2 * lprel + 1)) * threej * sixj2 * mosh1 * mosh2 * integral; // *2 factor is from the integral, was already included bt Ragnar
                                                                                                                                                                                                                                                                   // std::cout << "sixj1 = " << sixj1 << std::endl;
...
Operator Darwin(ModelSpace &modelspace, int Z)
{
    ...
    for (auto a : modelspace.all_orbits)
    {
        ...
        if (oa.l != 0)
            continue; // no spin-orbit in s-wave
...
Operator FirstOrderCorr_1b(const Operator &OpIn, const Operator &H)
{
    ...
    for (auto p : OpIn.modelspace->valence)
    {
        ...
        {
            if (std::find(OpIn.modelspace->valence.begin(), OpIn.modelspace->valence.end(), q) == OpIn.modelspace->valence.end())
                continue;
...
Operator RPA_resummed_1b(const Operator &OpIn, const Operator &H, std::string mode)
{
    ...
    for (auto h : OpIn.modelspace->holes)
    {
        ...
        for (auto p : OpIn.modelspace->particles)
        {
            ...
            if ((oh.j2 + op.j2) < 2 * Lambda or std::abs(oh.j2 - op.j2) > 2 * Lambda)
                continue;
            if ((oh.l + op.l + OpIn.GetParity()) % 2 > 0)
                continue;
            if (std::abs(oh.tz2 - op.tz2) != 2 * std::abs(OpIn.GetTRank()))
                continue;
            ph_kets.push_back(std::make_pair(p, h));
            hp_kets.push_back(std::make_pair(h, p));
            ph_phase.push_back(AngMom::phase((oh.j2 - op.j2) / 2));
    ...
    //                            [ Mhpph   Mhphp ]
    arma::mat M = arma::join_vert(arma::join_horiz(Mphph, Mphhp), arma::join_horiz(Mhpph, Mhphp));
...
double Calculate_r1xp2(ModelSpace &modelspace, Ket &bra, Ket &ket, int Jab, int Jcd)
{
    ...
    for (int Lab = std::abs(la - lb); Lab <= la + lb; ++Lab)
    {
        for (int Sab = 0; Sab <= 1; ++Sab)
        {
            if (std::abs(Lab - Sab) > Jab or Lab + Sab < Jab)
                continue;
    ...
    for (int ch = 0; ch < nchan; ++ch)
    {
        ...
        for (int ibra = 0; ibra < nkets; ++ibra)
        {
            ...
            for (int iket = ibra; iket < nkets; ++iket)
            {
                ...
                int fcd = 2 * nc + 2 * nd + lc + ld;
                if (fab != fcd)
                    continue; // L2 conserves all the quantum numbers
...
Operator BareDelta(ModelSpace &modelspace)
{
    ...
    for (int n = 0; n <= nmax; n++)
        psi_at_zero[n] = HO_Radial_psi(n, 0, hw, 0.0);
            ...
            for (int iket = ibra; iket < nkets; iket++)
            {
                ...
                for (int Lab = std::abs(la - lb); Lab <= la + lb; ++Lab)
                {
                    ...
                    for (int Sab = 0; Sab <= 1; ++Sab)
                    {
                        ...
                        int Lcd = Lab;
                        double njab = AngMom::NormNineJ(la, sa, ja, lb, sb, jb, Lab, Sab, J);
                        if (std::abs(njab) < 1e-8)
                            continue;
                        double njcd = AngMom::NormNineJ(lc, sc, jc, ld, sd, jd, Lcd, Scd, J);
                        if (std::abs(njcd) < 1e-8)
                            continue;
...
double MinnesotaMatEl(ModelSpace &modelspace, Ket &bra, Ket &ket, int J, const std::array<double, 6> &params)
{
    ...
    int fcd = 2 * nc + 2 * nd + lc + ld;
    if ((la + lb + lc + ld) % 2 != 0)
        return 0; // check parity conservation
                    ...
                    for (int lam_ab = (fab - 2 * N_ab - Lam_ab) % 2; lam_ab <= (fab - 2 * N_ab - Lam_ab); lam_ab += 2) // lam_ab = relative l for a,b
                    {
                        ...
                        for (int N_cd = N_ab; N_cd <= N_ab; ++N_cd) // N_cd = CoM n for c,d
                        {
                            ...
                            for (int p = pmin; p <= pmax; p++)
                            {
                                ...
                                if (Sab == 1)
                                    Ip += VT / pow(1.0 + kT, p + 1.5); // Triplet short-range potential
                                else
                                    Ip += VS / pow(1.0 + kS, p + 1.5); // Singlet short-range potential
                                vrel += Bp * Ip;
                            }
...
Operator EKKShift(Operator &Hin, int Nlower, int Nupper)
{
    ...
    for (auto i : index_lower)
        EKKShift.OneBody(i, i) = emid - elower;
    for (auto i : index_upper)
        EKKShift.OneBody(i, i) = emid - eupper;
...
double GetEmbeddedTBME(Operator &op1, index_t i, index_t j, index_t k, index_t l, int Jbra, int Jket, int Lambda)
{
    ...
    {
        if (j == l)
            embedded_tbme += OB(i, k);
        if (i == k)
            embedded_tbme += OB(j, l);
        if (i == l)
            embedded_tbme -= OB(j, k) * modelspace->phase(ji + jj - Jbra);
        if (j == k)
            embedded_tbme -= OB(i, l) * modelspace->phase(ji + jj - Jbra);
    }
    else // Tensor => slightly more complicated, tbmes are reduced.
    {
        if (j == l)
            embedded_tbme += OB(i, k) * modelspace->phase(ji + jj + Jket) * AngMom::SixJ(Jbra, Jket, Lambda, jk, ji, jj);
        if (i == k)
            embedded_tbme += OB(j, l) * modelspace->phase(jk + jl - Jbra) * AngMom::SixJ(Jbra, Jket, Lambda, jl, jj, ji);
        if (j == k)
            embedded_tbme -= OB(i, l) * modelspace->phase(ji + jj + jk + jl) * AngMom::SixJ(Jbra, Jket, Lambda, jl, ji, jj);
        if (i == l)
            embedded_tbme -= OB(j, k) * modelspace->phase(Jket - Jbra) * AngMom::SixJ(Jbra, Jket, Lambda, jk, jj, ji);
...
void CommutatorTest(Operator &X, Operator &Y)
{
    ...
    if ((X.IsHermitian() and Y.IsAntiHermitian()) or (X.IsAntiHermitian() and Y.IsHermitian()))
