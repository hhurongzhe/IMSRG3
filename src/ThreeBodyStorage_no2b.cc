...
ThreeBodyChannelNO2B::ThreeBodyChannelNO2B() : J2(-1), P2(-1), J1(-1), P1(-1), T3(-1), Ndim(-1) {}

ThreeBodyChannelNO2B::ThreeBodyChannelNO2B(int J2, int P2, int J1, int P1, int T3, ThreeBodySpaceNO2B &thr) : J2(J2), P2(P2), J1(J1), P1(P1), T3(T3)
{
    Ndim = 0;
    int emax = thr.Emax;
    int e2max = thr.E2max;
    ...
    for (int ia = 0; ia < Norbs; ia++)
    {
        ...
        for (int ib = ia; ib < Norbs; ib++)
        {
            ...
            if (oa.e + ob.e > e2max)
                continue;
            if (not AngMom::Triangle(oa.j, ob.j, 2 * J2))
                continue;
            ...
            for (int ic = 0; ic < Norbs; ic++)
            {
                ...
                for (int T2 : {0, 1})
                {
                    ...
                    if (not AngMom::Triangle(2 * T2, 1, T3))
                        continue;
                    if (((J2 + T2) % 2 == 0) and (ia == ib))
                        continue;
...
int ThreeBodyChannelNO2B::Hash_abct(int a, int b, int c, int Tab) const { return (a + (b << 10) + (c << 20) + (Tab << 30)); }
...
ThreeBodySpaceNO2B::ThreeBodySpaceNO2B(int emax, int e2max, int e3max, int lmax) : Emax(emax), E2max(e2max), E3max(e3max), Lmax(lmax)
{
...
int ThreeBodySpaceNO2B::Hash_Channel(int J2, int P2, int J1, int P1, int T3) const { return (J2 + (J1 << 8) + (P2 << 16) + (P1 << 17) + (T3 << 18)); }
...
template <class StoreType>
std::unique_ptr<ThreeBodyStorage> ThreeBodyStorage_no2b<StoreType>::Clone() const
{
    return std::unique_ptr<ThreeBodyStorage>(new ThreeBodyStorage_no2b<StoreType>(*this));
};
...
ThreeBodyStorage::ME_type ThreeBodyStorage_no2b<StoreType>::GetME_pn_no2b(int a, int b, int c, int d, int e, int f, int J2) const
{
    ...
    for (int T12 : {0, 1})
    {
        if (std::abs(oa.tz2 + ob.tz2) > 2 * T12)
            continue;
        for (int T45 : {0, 1})
        {
            if (std::abs(od.tz2 + oe.tz2) > 2 * T45)
                continue;
            //      for (int twoT=std::max( std::abs(2*T12-1), std::abs(2*T45-1) );
            for (int twoT = 1; twoT <= std::min(2 * T12 + 1, 2 * T45 + 1); twoT += 2)
            {
                vout += GetME_iso_no2b(i1, i2, i3, T12, i4, i5, i6, T45, J2, twoT) * AngMom::CG(0.5, oa.tz2 * 0.5, 0.5, ob.tz2 * 0.5, T12, (oa.tz2 + ob.tz2) * 0.5) * AngMom::CG(0.5, od.tz2 * 0.5, 0.5, oe.tz2 * 0.5, T45, (od.tz2 + oe.tz2) * 0.5) * AngMom::CG(T12, (oa.tz2 + ob.tz2) * 0.5, 0.5, oc.tz2 * 0.5, twoT * 0.5, Z3 * 0.5) * AngMom::CG(T45, (od.tz2 + oe.tz2) * 0.5, 0.5, of.tz2 * 0.5, twoT * 0.5, Z3 * 0.5);
            }
        }
    }
    return vout;
...
#pragma omp barrier
                                                ...
                                                if ((i1 == i2 and (J + T12) % 2 == 0) or (i4 == i5 and (J + T45) % 2 == 0))
                                                {
                                                    if (abs(vset) > 1.e-6)
                                                    {
                                                        ...
                                                        std::cout << std::setw(4) << i1 << std::setw(4) << i2 << std::setw(4) << i3 << std::setw(4) << T12 << std::setw(4) << i4 << std::setw(4) << i5 << std::setw(4) << i6 << std::setw(4) << T45 << std::setw(4) << J << std::setw(4) << T3 << std::setw(16) << counter << std::setw(12) << std::setprecision(6) << vset << std::endl;
                                                    }
                                                }
