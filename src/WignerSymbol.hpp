#pragma once
#ifndef UTIL_WIGNERSYMBOL_HPP
#define UTIL_WIGNERSYMBOL_HPP
...
constexpr uint64_t u64_factorial_data[] = {1,1,2,6,24,120,720,5040,40320,362880,3628800,39916800,479001600,6227020800,87178291200,1307674368000,20922789888000,355687428096000,6402373705728000,121645100408832000,2432902008176640000};
    // clang-format on
    ...
    inline double factorial(int n)
    {
        if (n < 0)
            return 0;
        if (n <= 20)
            return u64_factorial_data[n];
        double s = u64_factorial_data[20];
        for (int k = 21; k <= n; ++k)
        {
            s = s * k;
        }
    ...
    inline uint64_t u64_binomial(int n, int k)
    {
        if (unsigned(n) > unsigned(67) || unsigned(k) > unsigned(n))
            return 0;
        k = std::min(k, n - k);
        std::size_t x = n / 2 + 1;
        std::size_t index = x * (x - iseven(n)) + k;
        return u64_binomial_data[index];
    }
    ...
    public:
                ...
                for (int la = fa & 0x01; la <= fa; la += 2)
                {
                    ...
                    for (int lb = std::abs(l1 - la); lb <= std::min(la + l1, fb); lb += 2)
                    {
                        ...
                        for (int lc = std::abs(L - la); lc <= std::min(la + L, fc); lc += 2)
                        {
                            ...
                            const int nlc = nc + lc;
                            const double tc = ((uint64_t(2 * lc + 1) << lc) * unsafe_binomial(fc + 1, nc)) /
        ...
        void reserve(int num, std::string type, int rank)
        {
            if (type == "Jmax")
            {
                ...
                {
                case 3:
                    fill_binomial_data(3 * num + 1);
                    break;
                case 6:
                    fill_binomial_data(4 * num + 1);
                    break;
                case 9:
                    fill_binomial_data(5 * num + 1);
                    break;
                default:
                {
