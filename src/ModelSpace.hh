...
struct Orbit
{
    ...
    Orbit() {};
    Orbit(int n, int l, int j2, int tz2, double occ, int cvq, int index) : n(n), l(l), j2(j2), tz2(tz2), occ(occ), occ_nat(occ), cvq(cvq), index(index) {};
...
struct Ket //  | pq >
{
    ...
    // Methods
    Ket() {};
    Ket(Orbit &op_in, Orbit &oq_in) : op(&op_in), oq(&oq_in), p(op_in.index), q(oq_in.index), dpq(op_in.index == oq_in.index ? 1 : 0) { phase_prefactor = ((op->j2 + oq->j2) / 2 + 1) % 2 == 0 ? 1 : -1; };
...
struct Ket3 // | p q r >
{
    ...
    // Methods
    Ket3() {};
    Ket3(Orbit &op_in, Orbit &oq_in, Orbit &oR_in) : p(op_in.index), q(oq_in.index), r(oR_in.index), op(&op_in), oq(&oq_in), oR(&oR_in), Jpq(0) {};

    Ket3(Orbit &op_in, Orbit &oq_in, Orbit &oR_in, int jpq) : p(op_in.index), q(oq_in.index), r(oR_in.index), op(&op_in), oq(&oq_in), oR(&oR_in), Jpq(jpq) {};
...
class ModelSpace
{
  ...
  public:
    ...
    double GetCachedSixJ(int jj1, int jj2, int j_3, int jj4, int jj5, int j_6) { return six_j_cache_2b_.SixJ(jj1, jj2, j_3, jj4, jj5, j_6); }
