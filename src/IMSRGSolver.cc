...
IMSRGSolver::IMSRGSolver()
    : s(0), ds(0.1), ds_max(0.5), norm_domega(0.1), omega_norm_max(2.0), eta_criterion(1e-6), method("magnus_euler"), flowfile(""), n_omega_written(0), max_omega_written(500), magnus_adaptive(true), hunter_gatherer(false), perturbative_triples(false),
...
IMSRGSolver::IMSRGSolver(Operator &H_in)
    : modelspace(H_in.GetModelSpace()), H_0(&H_in), FlowingOps(1, H_in), Eta(H_in), istep(0), s(0), ds(0.1), ds_max(0.5), smax(2.0), norm_domega(0.1), omega_norm_max(2.0), eta_criterion(1e-6), method("magnus_euler"), flowfile(""), n_omega_written(0), max_omega_written(500), magnus_adaptive(true), hunter_gatherer(false), perturbative_triples(false),
      /*pert_triples_this_omega(0),pert_triples_sum(0),*/ ode_monitor(*this), ode_mode("H"), ode_e_abs(1e-6), ode_e_rel(1e-6)
{
    Eta.Erase();
...
void IMSRGSolver::NewOmega()
{
    ...
    std::cout << "pushing back another Omega. Omega.size = " << Omega.size() << " , operator size = " << Omega.front().Size() / 1024. / 1024. << " MB"
    ...
    if (scratchdir != "")
    {
        ...
        if (scratchdir.find("/dev/null") == std::string::npos)
        {
            ...
            {
                std::cout << "n_omega_written > max_omega_written.  (" << n_omega_written << " > " << max_omega_written << " ) deleting OMEGA files and calling terminate." << std::endl;
                CleanupScratch();
                std::terminate();
            }
        }
...
void IMSRGSolver::UpdateEta() { generator.Update(FlowingOps[0], Eta); }
...
Operator IMSRGSolver::Transform(Operator &OpIn) { return Transform_Partial(OpIn, 0); }
...
double IMSRGSolver::CalculatePerturbativeTriples()
{
    ...
    for (size_t n = 1; n < Omega.size(); n++)
        omega = BCH::BCH_Product(omega, Omega[n]);
...
void IMSRGSolver::WriteFlowStatus(std::ostream &f)
{
    if (f.good())
    {
        ...
        f << std::fixed << std::setw(5) << istep << std::setw(12) << std::setprecision(5) << s << std::setw(fwidth) << std::setprecision(fprecision) << H_s.ZeroBody << std::setw(fwidth) << std::setprecision(fprecision) << H_s.Norm() << std::setw(fwidth) << std::setprecision(fprecision) << cumulative_error << std::setw(fwidth) << std::setprecision(fprecision) << Omega.back().OneBodyNorm() << std::setw(fwidth) << std::setprecision(fprecision) << Omega.back().TwoBodyNorm() << std::setw(fwidth) << std::setprecision(fprecision) << Omega.back().ThreeBodyNorm()
          << std::setw(fwidth) << std::setprecision(fprecision) << Eta.OneBodyNorm() << std::setw(fwidth) << std::setprecision(fprecision) << Eta.TwoBodyNorm() << std::setw(fwidth) << std::setprecision(fprecision) << Eta.ThreeBodyNorm() << std::setw(7) << std::setprecision(0) << profiler.counter["N_ScalarCommutators"] + profiler.counter["N_TensorCommutators"] << std::setw(fwidth) << std::setprecision(fprecision) << H_s.GetMP2_Energy() << std::setw(7) << std::setprecision(0) << profiler.counter["N_Operators"] << std::setprecision(fprecision)
          << std::setw(12) << std::setprecision(3) << profiler.GetTimes()["real"] << std::setw(12) << std::setprecision(3) << profiler.CheckMem()["RSS"] / 1024. << " / " << std::skipws << profiler.MaxMemUsage() / 1024. << std::fixed << std::endl;
    }
}
...
void IMSRGSolver::WriteFlowStatusHeader(std::ostream &f)
{
    if (f.good())
    {
        ...
        int fprecision = 9;
        f.setf(std::ios::fixed);
        f << std::fixed << std::setw(5) << "i" << std::setw(12) << std::setprecision(5) << "s" << std::setw(fwidth) << std::setprecision(fprecision)
          << "E0"
          ...
          << std::setw(fwidth) << std::setprecision(fprecision) << " estim. err" << std::setw(fwidth) << std::setprecision(fprecision) << "||Omega_1||" << std::setw(fwidth) << std::setprecision(fprecision) << "||Omega_2||" << std::setw(fwidth) << std::setprecision(fprecision) << "||Omega_3||" << std::setw(fwidth) << std::setprecision(fprecision) << "||Eta_1||" << std::setw(fwidth) << std::setprecision(fprecision) << "||Eta_2||" << std::setw(fwidth) << std::setprecision(fprecision) << "||Eta_3||" << std::setw(7) << std::setprecision(fprecision) << "Ncomm"
          << std::setw(16) << std::setprecision(fprecision) << "E(MP2)" << std::setw(7) << std::setprecision(fprecision) << "N_Ops" << std::setw(16) << std::setprecision(fprecision) << "Walltime (s)" << std::setw(19) << std::setprecision(fprecision) << "Memory (MB)" << std::endl;
        for (int x = 0; x < 175; x++)
            f << "-";
        f << std::endl;
    }
