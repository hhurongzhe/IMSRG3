...
TwoBodyChannel_base::TwoBodyChannel_base(int j, int p, int t, ModelSpace *ms) : J(j), parity(p), Tz(t), modelspace(ms) {}

TwoBodyChannel_base::TwoBodyChannel_base(int ch, ModelSpace *ms) : modelspace(ms) {}
...
TwoBodyChannel::TwoBodyChannel(int j, int p, int t, ModelSpace *ms) : TwoBodyChannel_base(j, p, t, ms) { Initialize(); }
...
bool TwoBodyChannel::CheckChannel_ket(Orbit *op, Orbit *oq) const
{
    ...
    if ((op->l + oq->l) % 2 != parity)
        return false;
    if ((op->tz2 + oq->tz2) != 2 * Tz)
        return false;
    if (op->j2 + oq->j2 < 2 * J)
        return false;
    if (std::abs(op->j2 - oq->j2) > 2 * J)
        return false;
...
TwoBodyChannel_CC::TwoBodyChannel_CC(int j, int p, int t, ModelSpace *ms) : TwoBodyChannel_base(j, p, t, ms) { Initialize(); }
