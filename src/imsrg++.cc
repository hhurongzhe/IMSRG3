    ...
    */
        ...
        for (size_t i = 0; i < opnames.size(); ++i)
        {
            ...
            if (renormal_order)
            {
                if (op.GetParticleRank() > 2)
                    op.SetParticleRank(2); // Discard the residual 3N because we don't want to deal with it in the valence calculation
                op = op.UndoNormalOrdering();
                //        op.SetModelSpace(ms2);
                //        op = op.DoNormalOrdering();
    ...
    if (write_omega)
    {
        ...
        for (auto i : modelspace.all_orbits)
        {
            ...
            {
                file_occ << std::setw(wint) << oi.n << std::setw(wint) << oi.l << std::setw(wint) << oi.j2 << std::setw(wint) << oi.tz2 << std::setw(wdouble) << std::setiosflags(std::ios::fixed) << std::setprecision(pdouble) << std::scientific << oi.occ << std::endl;
            }
        }
        file_occ.close();
        if (basis == "NAT")
        {
            ...
            for (auto i : modelspace.all_orbits)
            {
                ...
                {
                    file_occ << std::setw(wint) << oi.n << std::setw(wint) << oi.l << std::setw(wint) << oi.j2 << std::setw(wint) << oi.tz2 << std::setw(wdouble) << std::setiosflags(std::ios::fixed) << std::setprecision(pdouble) << std::scientific << oi.occ_nat << std::endl;
                }
            }
            file_occ.close();
        }
