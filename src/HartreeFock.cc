...
HartreeFock::HartreeFock(Operator &hbare) : Hbare(hbare), modelspace(hbare.GetModelSpace()), ms_for_output_3N(hbare.GetModelSpace()), KE(Hbare.OneBody), energies(Hbare.OneBody.diag()), tolerance(1e-8), convergence_ediff(7, 0), convergence_EHF(7, 0), freeze_occupations(true), discard_NO2B_from_3N(false)
{
...
void HartreeFock::Solve()
{
    ...
    for (iterations = 0; iterations < maxiter; ++iterations)
    {
        ...
        if (iterations == 500)
        {
            ...
            std::cout << "Still not converged after 500 iterations. Setting density_mixing_factor => " << density_mixing_factor << " field_mixing_factor => " << field_mixing_factor << std::endl;
        }
    ...
    else
    {
        ...
        for (auto &x : convergence_ediff)
            std::cout << x << " ";
        std::cout << "  (tolerance = " << tolerance << ")" << std::endl;
        std::cout << "!!!! Last " << convergence_EHF.size() << "  EHF values: ";
        for (auto &x : convergence_EHF)
            std::cout << x << " ";
        std::cout << std::endl;
    }
    UpdateReference();
                ...
                for (uint64_t b : Hbare.OneBodyChannels.at({oa.l, oa.j2, oa.tz2}))
                {
                    ...
                    for (auto c : modelspace->all_orbits)
                    {
                        ...
                        for (uint64_t d : Hbare.OneBodyChannels.at({oc.l, oc.j2, oc.tz2}))
                        {
                            ...
                            if (eb + ed + ej > Hbare.E3max)
                                continue;
                            if ((oi.l + oa.l + ob.l + oj.l + oc.l + od.l) % 2 > 0)
                                continue;
...
#pragma omp atomic write
    ...
    std::cout << "HartreeFock::BuildMonopoleV3  storing " << Vmon3.size() << " doubles for Vmon3 and " << Vmon3_keys.size() << " uint64's for Vmon3_keys." << std::endl;
...
//*********************************************************************
uint64_t HartreeFock::Vmon3Hash(uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e, uint64_t f) { return a + (b << 10) + (c << 20) + (d << 30) + (e << 40) + (f << 50); }
...
//*********************************************************************
uint64_t HartreeFock::Vmon3JacobiHash(uint64_t na, uint64_t nb, uint64_t nc, uint64_t Jab, uint64_t twoJ, uint64_t E12, uint64_t alpha, uint64_t Ncm, uint64_t Lcm) { return (alpha + (na << 16) + (nb << 21) + (nc << 26) + (Jab << 31) + ((twoJ / 2) << 37) + (E12 << 44) + (Ncm << 51) + (Lcm << 57)); }
...
void HartreeFock::UpdateDensityMatrix_DIIS()
{
    ...
    if (nsave > N_MATS_STORED)
        DIIS_error_mats.pop_front();      // out with the old
    ...
    for (size_t i = 0; i < nsave; i++)
        rho += cvec(i) * DIIS_density_mats[i];
}
...
void HartreeFock::FillLowestOrbits()
{
    ...
    for (auto i : sorted_indices)
    {
        ...
        if ((placedZ >= refereceZ) and (placedN >= refereceN))
            break;
    }
...
void HartreeFock::UpdateF()
{
    ...
    for (auto it : Hbare.OneBodyChannels)
        onebodychannels.push_back(it.first);
...
void HartreeFock::ReorderCoefficients()
{
    for (auto &it : Hbare.OneBodyChannels)
    {
        ...
        {
            if (C(i, i) < 0)
                C.col(i) *= -1; // C is <HO|HF>, armadillow ordering is C(row,col)  so column j is the jth HF state
        }
    }
}
...
#pragma omp parallel for schedule(dynamic, 1) // confirmed that this improves performance
        for (int i = 0; i < npq; ++i)
        {
            ...
            for (int j = 0; j < npq; ++j)
            {
                ...
                {
                    Orbit &oa = modelspace->GetOrbit(a);
                    if (2 * oa.n + oa.l + e2bra > Hbare.GetE3max())
                        continue;
                    for (int b : Hbare.OneBodyChannels.at({oa.l, oa.j2, oa.tz2}))
                    {
                        Orbit &ob = modelspace->GetOrbit(b);
                        if (2 * ob.n + ob.l + e2ket > Hbare.GetE3max())
                            continue;
                        if (std::abs(rho(a, b)) < 1e-8)
                            continue; // Turns out this helps a bit (factor of 5 speed up in tests)
                        V3NO(i, j) += rho(a, b) * Hbare.ThreeBody.GetME_pn_no2b(bra.p, bra.q, a, ket.p, ket.q, b, J);
                    }
                }
                V3NO(i, j) /= (2 * J + 1);
                if (bra.p == bra.q)
                    V3NO(i, j) /= PhysConst::SQRT2;
                if (ket.p == ket.q)
                    V3NO(i, j) /= PhysConst::SQRT2;
                V3NO(j, i) = V3NO(i, j);
            }
...
void HartreeFock::PrintSPE()
{
    ...
    {
        Orbit &oi = modelspace->GetOrbit(i);
        std::cout << std::fixed << std::setw(3) << oi.n << " " << std::setw(3) << oi.l << " " << std::setw(3) << oi.j2 << " " << std::setw(3) << oi.tz2 << "   " << std::setw(10) << F_hfbasis(i, i) << std::endl;
    }
}
...
{
    arma::mat F_hfbasis = C.t() * F * C;
    std::cout << std::fixed << std::setw(3) << "i" << ": " << std::setw(3) << "n" << " " << std::setw(3) << "l" << " " << std::setw(3) << "2j" << " " << std::setw(3) << "2tz" << "   " << std::setw(12) << "SPE" << " " << std::setw(12) << "occ." << "   |   " << " overlaps" << std::endl;

    for (auto i : modelspace->all_orbits)
    {
        Orbit &oi = modelspace->GetOrbit(i);
        std::cout << std::fixed << std::setw(3) << i << ": " << std::setw(3) << oi.n << " " << std::setw(3) << oi.l << " " << std::setw(3) << oi.j2 << " " << std::setw(3) << oi.tz2 << "   " << std::setw(12) << std::setprecision(6) << F_hfbasis(i, i) << " " << std::setw(12) << oi.occ << "   | ";
...
#pragma omp parallel for schedule(dynamic, 1)
    for (size_t ich = 0; ich < nch3; ich++)
    {
        ...
        for (size_t ibra = 0; ibra < nbras_HF; ibra++)
        {
            ...
            int tz2j = bra.oq->tz2;
            int tz2k = bra.oR->tz2;
            if ((std::abs(ei - e_fermi[tz2i]) + std::abs(ej - e_fermi[tz2j]) + std::abs(ek - e_fermi[tz2k])) > ms_for_output_3N->GetdE3max())
                continue;
...
void HartreeFock::SetModelspaceForOutput3N(ModelSpace &ms) { ms_for_output_3N = &ms; }
