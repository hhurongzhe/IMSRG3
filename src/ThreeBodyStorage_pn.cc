...
void ThreeBodyStorage_pn::Allocate()
{
    ...
    for (size_t ch_bra = 0; ch_bra < nch; ch_bra++)
    {
        ...
        for (size_t ch_ket = ch_bra; ch_ket < nch; ch_ket++)
        {
            ...
            if ((std::abs(Tbc_bra.twoJ - Tbc_ket.twoJ) <= 2 * rank_J) and ((Tbc_bra.twoJ + Tbc_ket.twoJ) >= 2 * rank_J) and ((Tbc_bra.parity + Tbc_ket.parity) % 2 == parity) and (std::abs(Tbc_bra.twoTz - Tbc_ket.twoTz) == 2 * rank_T))
            {
    ...
    if (none_allocated)
    {
        std::cout << "DONE ALLOCATING PN 3-body, size of MatEl is " << MatEl.size() << "  ->  " << MatEl.size() * sizeof(pnME_type) / (1024. * 1024. * 1024.) << " GB" << std::endl;
    }
...
{
    if (!IsKetValid(Jab_in, twoJ, a, b, c) || !IsKetValid(Jde_in, twoJ, d, e, f))
        return 0.0;
    ...
    if (rank_J == 0 and rank_T == 0 and parity == 0 and (ch_bra != ch_ket))
        return 0;
    if (ch_bra == -1 or ch_ket == -1)
        return 0;
    ThreeBodyChannel &Tbc_bra = modelspace->GetThreeBodyChannel(ch_bra);
    ThreeBodyChannel &Tbc_ket = modelspace->GetThreeBodyChannel(ch_ket);
    if ((Tbc_bra.parity + Tbc_ket.parity) % 2 != parity)
        return 0;
    if (std::abs(Tbc_bra.twoTz - Tbc_ket.twoTz) != 2 * rank_T)
        return 0;
...
{
    if (!IsKetValid(Jab_in, j0, a, b, c) || !IsKetValid(Jde_in, j1, d, e, f))
        return 0.0;
    std::vector<double> recouple_bra;
    std::vector<double> recouple_ket;
    std::vector<size_t> index_bra;
    ...
    if (ch_bra == -1 or ch_ket == -1)
        return 0;
    if (std::abs(j0 - j1) > rank_J * 2 or (j0 + j1) < rank_J * 2)
        return 0;
    ThreeBodyChannel &Tbc_bra = modelspace->GetThreeBodyChannel(ch_bra);
    ThreeBodyChannel &Tbc_ket = modelspace->GetThreeBodyChannel(ch_ket);
    if ((Tbc_bra.parity + Tbc_ket.parity) % 2 != parity)
...
ThreeBodyStorage::ME_type ThreeBodyStorage_pn::GetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a, int b, int c, int d, int e, int f) const
{
    ...
    if (a == b and (Jab_in + tab_in) % 2 == 0)
        return 0;
    if (d == e and (Jde_in + tde_in) % 2 == 0)
        return 0;
    for (int tz2a : {-1, 1})
    {
        for (int tz2b : {-1, 1})
...
void ThreeBodyStorage_pn::Deallocate() { std::vector<ThreeBodyStorage_pn::pnME_type>().swap(MatEl); }

size_t ThreeBodyStorage_pn::size() const { return total_dimension * sizeof(pnME_type); }
...
void ThreeBodyStorage_pn::AccessME(size_t ch_bra, size_t ch_ket, size_t ibra, size_t iket, size_t &index, int &herm_flip) const
{
    ...
    if ((iter_ch_start == ch_start.end()) or iket_1 > ch_dim[ch_1] or iket_2 > ch_dim[ch_2])
    {
