...
ThreeBodyStorage_iso::ThreeBodyStorage_iso(const ThreeBodyStorage_iso &TBS_in) : ThreeBodyStorage(TBS_in), MatEl(TBS_in.MatEl), OrbitIndexHash(TBS_in.OrbitIndexHash), total_dimension(0) {}
...
void ThreeBodyStorage_iso::Allocate()
{
                ...
                for (int d = 0; d <= a; d += 2)
                {
                    ...
                    for (int e = 0; e <= (d == a ? b : d); e += 2)
                    {
                        ...
                        for (int f = 0; f <= ((d == a and e == b) ? c : e); f += 2)
                        {
                            ...
                            if ((ed + ee + ef) > E3max)
                                break;
                            if (((oa.l + ob.l + oc.l + od.l + oe.l + of.l) % 2 != parity) or (of.l > lmax))
                                continue;
    ...
    std::cout << "Allocated " << total_dimension << " three body matrix elements (" << total_dimension * sizeof(isoME_type) / 1024. / 1024. / 1024. << " GB), " << std::endl << "  number of buckets in hash table: " << OrbitIndexHash.bucket_count() << "  and load factor = " << OrbitIndexHash.load_factor() << "  estimated storage ~ " << ((OrbitIndexHash.bucket_count() + OrbitIndexHash.size()) * (sizeof(size_t) + sizeof(void *))) / (1024. * 1024. * 1024.) << " GB" << std::endl;
    is_allocated = true;
}
...
ThreeBodyStorage::ME_type ThreeBodyStorage_iso::GetME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b, int c, int d, int e, int f) const
{
    ...
    if (a == b and a == c and modelspace->GetOrbit(a).j2 < 3)
        return 0;
    if (d == e and d == f and modelspace->GetOrbit(d).j2 < 3)
        return 0;
    if (a == b and Jab_in % 2 > 0)
        return 0;
    if (d == e and Jde_in % 2 > 0)
        return 0;
...
void ThreeBodyStorage_iso::SetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a_in, int b_in, int c_in, int d_in, int e_in, int f_in, ThreeBodyStorage::ME_type V)
{
    ...
    for (auto elem : elements)
        me += MatEl.at(elem.first) * elem.second;
    for (auto elem : elements)
        MatEl.at(elem.first) += (V - me) * elem.second;
}
...
void ThreeBodyStorage_iso::AddToME_iso(int Jab, int Jde, int twoJ, int tab, int tde, int twoTabc, int twoTdef, int a, int b, int c, int d, int e, int f, ThreeBodyStorage::ME_type V)
{
    ...
    for (auto elem : elements)
        MatEl.at(elem.first) += V * elem.second;
}
...
size_t ThreeBodyStorage_iso::size() const { return total_dimension * sizeof(isoME_type); }
...
size_t ThreeBodyStorage_iso::KeyHash(size_t a, size_t b, size_t c, size_t d, size_t e, size_t f) const { return ((a / 2) + ((d / 2) << 10) + ((b / 2) << 20) + ((e / 2) << 30) + ((c / 2) << 40) + ((f / 2) << 50)); }
...
std::vector<std::pair<size_t, double>> ThreeBodyStorage_iso::AccessME(int Jab_in, int Jde_in, int twoJ, int tab_in, int tde_in, int twoTabc, int twoTdef, int a_in, int b_in, int c_in, int d_in, int e_in, int f_in) const
{
    ...
    Orbit &oe = modelspace->GetOrbit(e);
    Orbit &of = modelspace->GetOrbit(f);
    if (2 * (oa.n + ob.n + oc.n) + oa.l + ob.l + oc.l > E3max)
        return elements;
    if (2 * (od.n + oe.n + of.n) + od.l + oe.l + of.l > E3max)
        return elements;
            ...
            if (twoJ >= twoJ_min and twoJ <= twoJ_max and std::abs(Cj_abc * Cj_def) > 1e-8)
            {
                for (int tab = tab_min; tab <= tab_max; ++tab)
                {
                    ...
                    for (int tde = tde_min; tde <= tde_max; ++tde)
                    {
                        ...
                        if (std::abs(Ct_abc * Ct_def) < 1e-8)
                            continue;
                        if (herm == -1 and a == d and b == e and c == f and Jab == Jde and tab == tde)
                        ...
                        {
                            std::cout << "ThreeBodyStorage_iso::AccessME() " << __LINE__ << " --  AAAAHHH indx = " << indx << " + " << J_index << " + " << Tindex << " = " << indx + J_index + Tindex << "  but MatEl.size() = " << MatEl.size() << "    t values: " << tab << " " << tde << " " << twoTabc << " " << twoTdef << std::endl;
                            std::ostringstream oss;
