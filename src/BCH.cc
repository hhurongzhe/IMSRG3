...
void SetUseFactorizedCorrectionBCH_product(bool tf) { use_factorized_correction_BCH_Product = tf; }

void SetUseFactorized_Correct_ZBTerm(bool tf) { use_factorized_correct_ZBterm = tf; }
...
Operator BCH_Transform(const Operator &OpIn, const Operator &Omega) { return use_brueckner_bch ? Brueckner_BCH_Transform(OpIn, Omega) : Standard_BCH_Transform(OpIn, Omega); }
...
Operator Standard_BCH_Transform(const Operator &OpIn, const Operator &Omega)
{
    ...
    if (nx > bch_transform_threshold)
    {
        ...
        for (int i = 1; i <= max_iter; ++i)
        {
            ...
            if (OpOut.rank_J > 0)
            {
                std::cout << "Tensor BCH, i=" << i << "  Norm = " << std::setw(12) << std::setprecision(8) << std::fixed << OpNested.OneBodyNorm() << " " << std::setw(12) << std::setprecision(8) << std::fixed << OpNested.TwoBodyNorm() << " " << std::setw(12) << std::setprecision(8) << std::fixed << OpNested.ThreeBody.Norm() << " " << std::setw(12) << std::setprecision(8) << std::fixed << OpNested.Norm() << std::endl;
            }
            epsilon *= i + 1;
