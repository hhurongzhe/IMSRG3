...
ThreeBodyChannel::ThreeBodyChannel(int twoj, int p, int twotz, ModelSpace *ms) : twoJ(twoj), parity(p), twoTz(twotz), modelspace(ms) { Initialize(); }
...
void ThreeBodyChannel::Initialize()
{
    ...
    for (size_t iket3 = 0; iket3 < modelspace->Kets3.size(); iket3++)
    {
        ...
        if (CheckChannel_ket(ket3))
        {
            ...
            if (iket3 >= KetMap.size())
                KetMap.resize(2 * iket3 + 1, -1);
...
bool ThreeBodyChannel::CheckChannel_ket(Ket3 &ket3)
{
    ...
    if (std::abs(2 * ket3.Jpq - ket3.oR->j2) > twoJ or (2 * ket3.Jpq + ket3.oR->j2) < twoJ)
        return false; // triangle condition
                      // TODO Put these back in. This is only for testing...
    if (twoJ == (ket3.op->j2 + ket3.oq->j2 + ket3.oR->j2))
    {
        if (ket3.p == ket3.q or ket3.p == ket3.r or ket3.q == ket3.r)
            return false;
    }
    if (twoJ == (ket3.op->j2 + ket3.oq->j2 + ket3.oR->j2) and (ket3.p == ket3.q or ket3.q == ket3.r))
    ...
    if ((ket3.q == ket3.r) and std::abs(twoJ - ket3.op->j2) == 2 * ket3.oq->j2)
        return false; // fully stretched state can't have p==q
    if ((ket3.p == ket3.q) and (ket3.p == ket3.r) and twoJ > (3 * ket3.op->j2 - 3))
        return false;
    return true;
}

Ket3 &ThreeBodyChannel::GetKet(size_t iket) { return modelspace->GetKet3(KetList[iket]); };
