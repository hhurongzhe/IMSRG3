...
{
    if (round(2 * (j1 + j2)) < round(2 * j3))
        return false;
    if (std::abs(round(2 * (j1 - j2))) > round(2 * j3))
        return false;
    return true;
}
...
{
    return util::wigner_3j(int(2 * j1), int(2 * j2), int(2 * j3), int(2 * m1), int(2 * m2), int(2 * m3));
    // return gsl_sf_coupling_3j(int(2 * j1), int(2 * j2), int(2 * j3), int(2 * m1), int(2 * m2), int(2 * m3));
}

/// Clebsch-Gordan coefficient
double CG(double ja, double ma, double jb, double mb, double J, double M) { return phase(ja - jb + M) * sqrt(2 * J + 1) * ThreeJ(ja, jb, J, ma, mb, -M); }

/// Wigner 6j symbol
double SixJ(double j1, double j2, double j3, double J1, double J2, double J3)
{
    return util::wigner_6j(int(2 * j1), int(2 * j2), int(2 * j3), int(2 * J1), int(2 * J2), int(2 * J3));
...
{
    return util::wigner_9j(int(2 * j1), int(2 * j2), int(2 * J12), int(2 * j3), int(2 * j4), int(2 * J34), int(2 * J13), int(2 * J24), int(2 * J));
...
double NormNineJ(double j1, double j2, double J12, double j3, double j4, double J34, double J13, double J24, double J) { return sqrt((2 * J12 + 1) * (2 * J34 + 1) * (2 * J13 + 1) * (2 * J24 + 1)) * NineJ(j1, j2, J12, j3, j4, J34, J13, J24, J); }
...
double TwelveJ_1_ints(int a1, int a2, int a3, int a4, int b12, int b23, int b34, int b41, int c1, int c2, int c3, int c4)
{
    ...
    {
        double sixj1 = util::wigner_6j(a1, a2, b12, c2, c1, twoX);
        double sixj2 = util::wigner_6j(a2, a3, b23, c3, c2, twoX);
        double sixj3 = util::wigner_6j(a3, a4, b34, c4, c3, twoX);
        double sixj4 = util::wigner_6j(a4, a1, b41, c1, c4, twoX);
...
double TwelveJ_1(double a1, double a2, double a3, double a4, double b12, double b23, double b34, double b41, double c1, double c2, double c3, double c4) { return AngMom::TwelveJ_1_ints(a1 * 2, a2 * 2, a3 * 2, a4 * 2, b12 * 2, b23 * 2, b34 * 2, b41 * 2, c1 * 2, c2 * 2, c3 * 2, c4 * 2); }
        ...
        for (int lb = (la + l1) % 2; lb <= std::min(l1 + la, f1 - la); lb += 2)
        {
            ...
            for (int lc = (la + L) % 2; lc <= std::min(la + L, F - la); lc += 2)
            {
                ...
                for (int ld = (lb + l) % 2; ld <= ldmax; ld += 2)
                {
                    ...
                    double mosh3_pre = phase(la + lb + lc) * pow(2., (la + lb + lc + ld) / 2.) * ninej * cg_ab * cg_ac * cg_bd * cg_cd * (2 * la + 1) * (2 * lb + 1) * (2 * lc + 1) * (2 * ld + 1);
                    double mosh3 = 0;
                    for (int a = 0; a <= amax; ++a)
                    {
                        ...
                        mosh3 += pow(sB, 2 * a + la + 2 * d + ld) * pow(cB, 2 * b + lb + 2 * c + lc) / (factorial(a) * double_fact(2. * (a + la) + 1) * factorial(b) * double_fact(2. * (b + lb) + 1) * factorial(c) * double_fact(2. * (c + lc) + 1) * factorial(d) * double_fact(2. * (d + ld) + 1));
                    }
...
double TalmiB(int n, int l, int nn, int ll, int p)
{
    ...
    {
        B += factorial(l + k) * factorial(p - (l - ll) / 2 - k) / (factorial(k) * factorial(2 * l + 2 * k + 1) * factorial(n - k) * factorial(2 * p - l + ll - 2 * k + 1) * factorial(nn - p + (l + ll) / 2 + k) * factorial(p - (l + ll) / 2 - k));
    }
    B *= phase(p - (l + ll) / 2) * factorial(2 * p + 1) / (pow(2, n + nn) * factorial(p)) * sqrt(factorial(n) * factorial(nn) * factorial(2 * n + 2 * l + 1) * factorial(2 * nn + 2 * ll + 1) / (factorial(n + l) * factorial(nn + ll)));
                ...
                for (int L = L_min; L <= L_max; L += 1)
                {
                    ...
                    for (int curlyL = curlyL_min; curlyL <= curlyL_max; curlyL += 1)
                    {
                        ...
                        for (int Lambda = Lambda_min; Lambda <= Lambda_max; Lambda += 1)
                        {
                            ...
                            if ((2 * na + la + 2 * nb + lb + 2 * nc + lc) != (2 * Ncm + Lcm + 2 * N1 + L1 + 2 * N2 + L2))
                                continue;
...
double Tcoeff(int na, int la, int j2a, int nb, int lb, int j2b, int nc, int lc, int j2c, int Jab, int twoJ, int N1, int L1, int S1, int J1, int N2, int L2, int twoJ2, int twoJ12, int Ncm, int Lcm)
{
    ...
    for (int Lab = Lab_min; Lab <= Lab_max; Lab++)
    {
        ...
        if (std::abs(ninejab) < 1e-8)
            continue; // <- This may not help very much
        ...
        for (int L12 = L12_min; L12 <= L12_max; L12 += 1)
        {
            ...
            for (int L = L_min; L <= L_max; L++)
            {
                ...
                {
                    t12 += (twoS12 + 1) * NineJ(L1, L2, L12, S1, S2, 0.5 * twoS12, J1, 0.5 * twoJ2, 0.5 * twoJ12) * NineJ(Lab, lc, L, S1, sc, 0.5 * twoS12, Jab, jc, 0.5 * twoJ) * SixJ(Lcm, L12, L, 0.5 * twoS12, 0.5 * twoJ, 0.5 * twoJ12) * phase(lc + Lab + L + L1 + (twoJ + twoS12) / 2);
                }
                if (std::abs(t12) < 1e-9)
                    continue;
                ...
                for (int Lambda = Lambda_min; Lambda <= Lambda_max; Lambda++)
                {
                    ...
                    for (int curlyL = curlyL_min; curlyL <= curlyL_max; curlyL += 1)
                    {
                        ...
                        if (curlyN < 0)
                            continue;
                        if ((la + lb + L1 + curlyL) % 2 > 0)
                            continue;
                        if (2 * curlyN + curlyL + 2 * nc + lc != 2 * Ncm + Lcm + 2 * N2 + L2)
                            continue;
                        ...
                        tmosh += sixjLambda * sixjCurly * phase(Lambda) * moshphase1 * moshphase2 * Moshinsky(curlyN, curlyL, N1, L1, na, la, nb, lb, Lab, MOSH_BETA_1) * Moshinsky(Ncm, Lcm, N2, L2, curlyN, curlyL, nc, lc, Lambda, MOSH_BETA_2);
                    }
                }
