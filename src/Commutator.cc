...
namespace
{
  bool ConcurrentBlasSafe()
  {
#ifdef OPENBLAS_VERSION
    // SINGLE_THREADED OpenBLAS builds may lack USE_LOCKING. Concurrent
    // GEMMs then corrupt results even with OPENBLAS_NUM_THREADS=1.
    // Conservatively serialize these loops when locking cannot be verified.
...
#pragma omp parallel for schedule(dynamic) reduction(+:z0) if(ConcurrentBlasSafe())
    for (int ich = 0; ich < nch; ++ich)
    {
      ...
      const auto& X2 = X.TwoBody.GetMatrix(ch_bra,ch_ket).rows(hh);
	...
	for (size_t i=0;i<ph_bra.size(); i++)
	{
        ...
        const auto& X2 = X.TwoBody.GetMatrix(ch_bra,ch_ket).cols(hh);
  ...
  void ConstructScalarMpp_Mhh(const Operator &X, const Operator &Y, const Operator &Z, TwoBodyME &Mpp, TwoBodyME &Mhh)
  {
    ...
    #pragma omp parallel for schedule(dynamic, 1) if(ConcurrentBlasSafe())
    #endif
    for (int ich = 0; ich < nch; ++ich)
    {
