...
{
    for (std::string term : {"comm330ss", "comm331ss", "comm231ss", "comm132ss", "comm232ss", "comm332_ppph_hhhpss", "comm332_pphhss", "comm133ss", "comm223ss", "comm233_pp_hhss", "comm233_phss", "comm333_ppp_hhhss", "comm333_pph_hhpss"})
    {
...
void SetUseIMSRG3N7(bool tf)
{
    ...
    if (use_imsrg3_n7)
    {
        for (std::string term : {"comm332_ppph_hhhpss", "comm332_pphhss", "comm233_pp_hhss", "comm233_phss", "comm333_ppp_hhhss", "comm333_pph_hhpss"})
        {
            comm_term_on[term] = false;
        }
        for (std::string term : {"comm330ss", "comm331ss", "comm231ss", "comm132ss", "comm232ss", "comm133ss", "comm223ss"})
        {
...
void SetUseIMSRG3N7_Tensor(bool tf)
{
    use_imsrg3_n7 = tf;
    use_imsrg3 = tf;
    for (std::string term : {"comm331st", "comm231st", "comm132st", "comm232st", "comm133st", "comm223st"})
    {
        comm_term_on[term] = tf;
    }
}
...
void SetUseIMSRG3_MP4(bool tf)
{
    if (tf)
    {
        for (std::string term : {"comm331ss", "comm231ss", "comm132ss", "comm332_ppph_hhhpss", "comm332_pphhss", "comm233_pp_hhss", "comm233_phss", "comm333_ppp_hhhss", "comm333_pph_hhpss"})
        {
            comm_term_on[term] = false;
        }
        for (std::string term : {"comm330ss", "comm232ss", "comm133ss", "comm223ss"})
        {
            comm_term_on[term] = true;
        }
    }
...
void SetSingleThread(bool tf) { single_thread = tf; }

void SetVerbose(bool tf) { verbose = tf; }
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int ich = 0; ich < n_nonzero; ++ich)
    {
        ...
        for (int indx_ij = 0; indx_ij < npq; ++indx_ij)
        {
            ...
            {
                W2.col(indx_ij) = join_horiz(Y2.cols(join_vert(u_ind2_aj, u_ind2_ai)), X2.cols(join_vert(u_ind2_aj, u_ind2_ai))) * join_vert(join_vert(X1.unsafe_col(i).rows(u_ind1_ia) % v_factor_ia, X1.unsafe_col(j).rows(u_ind1_ja) % v_factor_ja), -join_vert(Y1.unsafe_col(i).rows(u_ind1_ia) % v_factor_ia, Y1.unsafe_col(j).rows(u_ind1_ja) % v_factor_ja));
            }
    ...
    if (not(X.GetParity() == 0 and Y.GetParity() == 0 and Z.GetParity() == 0 and X.GetTRank() == 0 and Y.GetTRank() == 0 and Z.GetTRank() == 0))
    {
        ...
        else
        {
            std::cout << " Uh Oh. " << __func__ << " line " << __LINE__ << "  not supported with these two operators: "
                      << "  parity " << X.GetParity() << " " << Y.GetParity() << "  isospin " << X.GetTRank() << " " << Y.GetTRank() << "  I quit." << std::endl;
            std::exit(EXIT_FAILURE);
        }
        Z.MakeNotReduced();
        return;
