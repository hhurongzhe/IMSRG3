...
#pragma omp parallel for schedule(dynamic, 1)
    for (size_t index_ij = 0; index_ij < nij; index_ij++)
    {
        ...
        for (int xy_iter = 0; xy_iter <= 1; xy_iter++)
        {
            ...
            if (OP3.ThreeBodyNorm() < 1e-8)
                continue; // no use if there isn't a 3body piece
...
void comm232ss_srs_optimized(const Operator &X, const Operator &Y, Operator &Z)
{
    ...
    auto Hash_obc_index = [](int j2, int parity, int tz2) { return 2 * j2 - 1 + tz2 + parity; };
        ...
        for (size_t local_index_bra = 0; local_index_bra < nbras_kept; local_index_bra++)
        {
            ...
            for (size_t iperm = 0; iperm < 3; iperm++)
            {
                ...
                for (auto a : X.GetOneBodyChannel(oreplace.l, oreplace.j2, oreplace.tz2))
                {
                    ...
                    if (ch_return != ch_internal_xy)
                        ket_list.clear(); // if this ket is in the wrong channel, don't add it. (This can happen for isospin changing operators).
...
bool check_2b_channel_Tz_parity(const Operator &Op, Orbit &o1, Orbit &o2, Orbit &o3, Orbit &o4) { return (((o1.l + o2.l + o3.l + o4.l) % 2 == Op.parity) and (std::abs(o1.tz2 + o2.tz2 - o3.tz2 - o4.tz2) == 2 * Op.rank_T)); }
...
void comm233_phss(const Operator &X, const Operator &Y, Operator &Z)
{
    ...
    auto unhash_key_lmij = [](size_t &l, size_t &m, size_t &Jlm, size_t &i, size_t &j, size_t &Jij, size_t key)
    {
        ...
        Jlm = ((key >> 48) & 0xFFL);
        Jij = ((key >> 56) & 0xFFL);
    };                                                                       //  0xF = 15 = 1111 (4bits), so 0xFFF is 12 bits of 1's. 0xFFFL makes it a long
        ...
        for (auto &iter_ijn : good_kets_ijn)
        {
            ...
            for (size_t Iabc = 0; Iabc < ngood_abc; Iabc++)
            {
                ...
                for (int twoJprime = twoJprime_min; twoJprime <= twoJprime_max; twoJprime += 2)
                {
                    ...
                    double sixj_ijn = (2 * Jph < twoJprime) ? Z.modelspace->GetSixJ(jn, Jph, Jij, jc, 0.5 * twoJprime, Jab) : Z.modelspace->GetSixJ(jn, 0.5 * twoJprime, Jab, jc, Jph, Jij);
