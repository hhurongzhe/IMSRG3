...
uint64_t AHash(int index_p, int index_pp, int J, int norm) { return (((uint64_t)(index_p)) << 30) + (((uint64_t)(index_pp)) << 20) + (((uint64_t)(J)) << 10) + ((uint64_t)(norm)); }
...
uint64_t IntHash(int n, int l, int np, int lp, int J) { return (((uint64_t)(n)) << 40) + (((uint64_t)(l)) << 30) + (((uint64_t)(np)) << 20) + (((uint64_t)(lp)) << 10) + ((uint64_t)(J)); }
...
#pragma omp parallel for schedule(dynamic, 1) // this works as long as the gsl_function handle is within this for-loop
    ...
    std::cout << "IntList has " << IntList.bucket_count() << " buckets and a load factor " << IntList.load_factor() << ", estimated storage ~= " << ((IntList.bucket_count() + IntList.size()) * (sizeof(size_t) + sizeof(void *))) / (1024.0 * 1024.0 * 1024.0) << " GB" << std::endl; // copied from (RS)
    ...
    else // if we didn't find it, calculate it and add it to the list!
    {
        ...
        if (omp_get_num_threads() >= 2)
        {
            ...
            std::cout << "DANGER!!!!!!!  Updating IntList inside a parellel loop breaks thread safety!" << std::endl
                      ...
                      << "key = " << std::hex << key << std::dec << "   integral = " << integral << std::endl;
            //        printf("DANGER!!!!!!!  Updating IntList inside a parellel loop breaks thread safety!\n");
                        ...
                        for (int lr = 1; lr <= eps_ab; lr++)
                        {
                            for (int nr = 0; nr <= (eps_ab - lr) / 2; nr++)
                            {
                                ...
                                for (int Ncom = 0; Ncom <= tempmaxNcom; Ncom++)
                                {
                                    ...
                                    if ((Lam + 2 * Ncom) > eps_cd)
                                        continue;
                                    if ((std::abs(Lam - lr) > Lf) or ((Lam + lr) < Lf))
                                        continue;
                                    if ((lr + Lam + eps_ab) % 2 > 0)
                                        continue;
