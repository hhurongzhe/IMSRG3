...
Jacobi3BME::Jacobi3BME(int nmax, int twojmin, int twojmax, int twotmin, int twotmax) : Nmax(nmax), twoJmin(twojmin), twoJmax(twojmax), twoTmin(twotmin), twoTmax(twotmax)
{
                ...
                for (int Nbra = parity; Nbra <= Nmax; Nbra += 2)
                {
                    ...
                    for (size_t i2 = 0; i2 < jacobi_2.size(); i2++)
                    {
                        ...
                        for (size_t i1 = 0; i1 < jacobi_1.size(); i1++)
                        {
                            ...
                            if (std::abs(jac1.t * 2 - t) != 1)
                                continue; // isospin triangle
                            if (std::abs(jac1.j * 2 - j) > jac2.j2 or (jac1.j * 2 + j) < jac2.j2)
...
// and N%2 = parity
size_t Jacobi3BME::HashTJN(int twoT, int twoJ, int N) { return (twoT - twoTmin) / 2 * ((twoJmax - twoJmin) / 2 * (Nmax + 1) + Nmax + 1) + (twoJ - twoJmin) / 2 * (Nmax + 1) + (N); }
...
size_t Jacobi3BME::HashTJNN(int twoT, int twoJ, int Nbra, int Nket)
{
    ...
    return (twoT - twoTmin) / 2 * ((twoJmax - twoJmin) / 2 * (Nmax / 2 * (Nround + 1) + Nround + 1) + (Nmax / 2) * (Nround + 1) + Nround + 1) + (twoJ - twoJmin) / 2 * (Nmax / 2 * (Nround + 1) + Nround + 1) + (Nbra / 2) * (Nround + 1) + (Nket / 2) * 2 + (Nket % 2); ///  N2%2 gets the parity, beyond that, whether N1 and N2 are even or odd is redundant information
}
...
double Jacobi3BME::GetLabMatEl(Ket3 &bra, Ket3 &ket, int Jab, int Jde, int twoJ, int Tab, int Tde, int twoT)
{
    ...
    for (int Ecm = 0; Ecm <= std::min(Ebra, Eket); Ecm++)
    {
        ...
        if (E12_bra > Nmax or E12_ket > Nmax)
            continue;
        ...
        for (int Lcm = (Ecm % 2); Lcm <= Ecm; Lcm += 2)
        {
            ...
            for (int twoJ12 = twoJ12_min; twoJ12 <= twoJ12_max; twoJ12 += 2) // the interaction conserves J12, parity, and T12=twoT
            {
                ...
                for (int ibraNAS = 0; ibraNAS < NASdim_bra; ibraNAS++)
                {
                    if (std::abs(Tcoeff_bra[ibraNAS]) < 1e-9)
                        continue;
                    jacobi1_state jac1_bra, jac1_ket;
                    jacobi2_state jac2_bra, jac2_ket;
                    ...
                    for (int iketNAS = 0; iketNAS < NASdim_ket; iketNAS++)
                    {
                        if (std::abs(Tcoeff_ket[iketNAS]) < 1e-9)
                            continue;
...
void Jacobi3BME::GetV3mon_all(HartreeFock &hf)
{
    ...
    {
        ljChannel ljchan = {obc.first[0], obc.first[1]};
        if (std::find(ljchannels.begin(), ljchannels.end(), ljchan) == ljchannels.end())
    ...
    for (size_t ilj_a = 0; ilj_a < num_lj; ilj_a++)
    {
        ...
        for (auto &orb : hf.modelspace->Orbits)
            if (orb.l == la and orb.j2 == j2a)
                na_list.insert(orb.n);
        ...
        for (size_t ilj_b = ilj_a; ilj_b < num_lj; ilj_b++) // TODO: change the lower limit from 0 to ilj_a
        {
            ...
            for (auto &orb : hf.modelspace->Orbits)
                if (orb.l == lb and orb.j2 == j2b)
                    nb_list.insert(orb.n);
            int Jab_min = std::abs(j2a - j2b) / 2;
            int Jab_max = (j2a + j2b) / 2;
            for (size_t ilj_c = 0; ilj_c < num_lj; ilj_c++) // TODO: possibly also change lower limit from 0 to ilj_b, although this becomes more complicated, due to recoupling
            {
                ...
                for (auto &orb : hf.modelspace->Orbits)
                    if (orb.l == lc and orb.j2 == j2c)
                        nc_list.insert(orb.n);
                            ...
                            for (int Tab = Tab_min; Tab <= 1; Tab++)
                            {
                                ...
                                for (int Tde = Tab_min; Tde <= 1; Tde++)
                                {
                                    ...
                                    for (int twoT = twoT_min; twoT <= std::min(2 * Tab + 1, 2 * Tde + 1); twoT += 2)
                                    {
                                        ...
                                        double isoClebsch_c = isospin3_Clebsch[2 * Tab + twoT / 2];
                                        double isoClebsch_f = isospin3_Clebsch[2 * Tde + twoT / 2];
                                        if (std::abs(isoClebsch_c) < 1e-6 or std::abs(isoClebsch_f) < 1e-6)
                                            continue;
                                        double v_sumJT = 0;
                                        for (int Ecm = 0; Ecm <= std::min(Eabc, Edef); Ecm++)
                                        {
                                            ...
                                            for (int Lcm = Ecm % 2; Lcm <= Ecm; Lcm += 2)
                                            {
                                                ...
                                                {
                                                    if (verbose)
                                                        std::cout << "twoJ,Lcm,twoJ12,twoJmax = " << twoJ << " " << Lcm << " " << twoJ12 << " " << twoJmax << std::endl;
                                                    if (verbose)
                                                        std::cout << "twoT,Tab,Tde = " << twoT << " " << Tab << " " << Tde << std::endl;
                                                    if (verbose)
                                                        std::cout << "Ecm,E12abc,E12def " << Ecm << " " << E12abc << " " << E12def << std::endl;
                                                    auto hashTJN_abc = HashTJN(twoT, twoJ12, E12abc);
                                                    auto hashTJN_def = HashTJN(twoT, twoJ12, E12def);
                                                    //                       if (verbose) std::cout << "hash_abc = " << hashTJN_abc << " hash_def = " << hashTJN_def << "  size of dimNAS = " << dimensionNAS.size() << std::endl;
                                                    size_t dimNAS_abc = GetDimensionNAS(twoT, twoJ12, parity, E12abc);
                                                    ...
                                                    //                       std::cout << "Getting matrix beginning at " << startloc << "  ( should it be " << otherstartloc << " ? ) " <<  std::endl;
                                                    if (verbose)
                                                        std::cout << "Getting matrix beginning at " << startloc << "   size of vector = " << meNAS.size() << std::endl;
                                                    for (size_t iNAS_abc = 0; iNAS_abc < dimNAS_abc; iNAS_abc++)
                                                    {
                                                        auto &index_1_2_abc = NAS_jacobi_states.at(hashTJN_abc).at(iNAS_abc);
...
double Jacobi3BME::Tcoeff_wrapper(Ket3 &ket, int Jab, int twoJ, jacobi1_state &jac1, jacobi2_state &jac2, int twoJ12, int Ncm, int Lcm) { return AngMom::Tcoeff(ket.op->n, ket.op->l, ket.op->j2, ket.oq->n, ket.oq->l, ket.oq->j2, ket.oR->n, ket.oR->l, ket.oR->j2, Jab, twoJ, jac1.n, jac1.l, jac1.s, jac1.j, jac2.n, jac2.l, jac2.j2, twoJ12, Ncm, Lcm); }
                        ...
                        for (int jb = std::abs(2 * lb - 1); jb <= 2 * lb + 1; jb += 2)
                        {
                            for (int lc = (ntot + la + lb) % 2; lc <= ntot - la - lb; lc += 2)
                            {
                                for (int jc = std::abs(2 * lc - 1); jc <= 2 * lc + 1; jc += 2)
                                {
                                    ...
                                    {
                                        if (std::abs(2 * bj12 - jc) > jtot or (2 * bj12 + jc) < jtot)
                                            continue;
                                        for (int na = 0; na <= (ntot - la - lb - lc) / 2; na++)
                                        {
                                            for (int nb = 0; nb <= (ntot - la - lb - lc) / 2; nb++)
                                            {
                                                ...
                                                if (nc < 0)
                                                    continue;
                                                if (ntot != 2 * (na + nb + nc) + la + lb + lc)
                                                    continue;
                                                if (((2 * na + la) > emax) or ((2 * nb + lb) > emax) or ((2 * nc + lc) > emax))
                                                    continue;
                                                if (((2 * na + la + 2 * nb + lb) > E2max) or ((2 * na + la + 2 * nc + lc) > E2max) or ((2 * nb + lb + 2 * nc + lc) > E2max))
                                                    continue;
                                    ...
                                    for (int s12 = 0; s12 <= 1; s12++)
                                    {
                                        for (int l12 = std::abs(s12 - j12); l12 <= s12 + j12; l12++)
                                        {
                                            ...
                                            for (int n12 = 0; n12 <= (ntot - 2 * ncm - l12 - l3 - lcm) / 2; n12++)
                                            {
                                                ...
                                                if (n3 < 0 or ((2 * n3 + 2 * n12 + 2 * ncm + l3 + l12 + lcm) != ntot))
        ...
        for (int ntot = std::max((jtot - 3) / 2, 0); ntot <= Nmax; ntot++) // total harmonic oscillator quanta e1+e2+e3
        {
            ...
            for (int isp = bk.nspstart; isp < bk.nspstart + bk.nspnum; isp++)
            {
                ...
                for (int irel = bk.nrelstart; irel < bk.nrelstart + bk.nrelnum; irel++)
                {
                    ...
                    double mytc = AngMom::Tcoeff(lab.na, lab.la, lab.ja, lab.nb, lab.lb, lab.jb, lab.nc, lab.lc, lab.jc, lab.bj12, jtot, rel.n12, rel.l12, rel.s12, rel.j12, rel.n3, rel.l3, rel.I3, rel.j3, rel.ncm, rel.lcm);
                    double mytc_bf = AngMom::Tcoeff_bruteforce(lab.na, lab.la, lab.ja, lab.nb, lab.lb, lab.jb, lab.nc, lab.lc, lab.jc, lab.bj12, jtot, rel.n12, rel.l12, rel.s12, rel.j12, rel.n3, rel.l3, rel.I3, rel.j3, rel.ncm, rel.lcm);
                    ...
                    std::cout << "( " << lab.na << " " << lab.la << " " << lab.ja << " " << lab.nb << " " << lab.lb << " " << lab.jb << " " << lab.nc << " " << lab.lc << " " << lab.jc << " " << lab.bj12 << " " << jtot << " " << rel.n12 << " " << rel.l12 << " " << rel.s12 << " " << rel.j12 << " " << rel.n3 << " " << rel.l3 << " " << rel.I3 << " " << rel.j3 << " " << rel.ncm << " " << rel.lcm << " ) " << std::endl;
                    std::cout << "   " << isp << " " << irel << "  " << index << " ( " << tcoeff.size() << " )  -> " << std::fixed << std::setw(12) << tc << "   " << std::fixed << std::setw(12) << mytc << "   " << std::fixed << std::setw(12) << mytc_bf << "  " << std::fixed << std::setw(8) << good1 << std::fixed << std::setw(8) << good2 << std::endl;
                }
            }
        }
    }
