...
ThreeBodyStorage::ThreeBodyStorage(ModelSpace *ms) : modelspace(ms), emax(ms->GetEMax3Body()), E2max(ms->GetE3max()), E3max(ms->GetE3max()), lmax(ms->GetLmax()) {}

ThreeBodyStorage::ThreeBodyStorage(ModelSpace *ms, int e3max) : modelspace(ms), emax(ms->GetEMax3Body()), E2max(ms->GetE2max()), E3max(ms->GetE3max()), lmax(ms->GetLmax()) {}

ThreeBodyStorage::ThreeBodyStorage(const ThreeBodyStorage &TBS_in) : modelspace(TBS_in.modelspace), emax(TBS_in.emax), E2max(TBS_in.E2max), E3max(TBS_in.E3max), lmax(TBS_in.lmax), herm(TBS_in.herm), rank_J(TBS_in.rank_J), rank_T(TBS_in.rank_T), parity(TBS_in.parity), ISOSPIN_BLOCK_DIMENSION(TBS_in.ISOSPIN_BLOCK_DIMENSION), is_allocated(TBS_in.is_allocated), ch_start(TBS_in.ch_start), ch_dim(TBS_in.ch_dim) {}

ThreeBodyStorage::ThreeBodyStorage(ModelSpace *ms, int rJ, int rT, int p) : modelspace(ms), emax(ms->GetEMax3Body()), E2max(ms->GetE2max()), E3max(ms->GetE3max()), lmax(ms->GetLmax()), rank_J(rJ), rank_T(rT), parity(p) {}

ThreeBodyStorage::ThreeBodyStorage(ModelSpace *ms, int e3max, int rJ, int rT, int p) : modelspace(ms), emax(ms->GetEMax3Body()), E2max(ms->GetE2max()), E3max(e3max), lmax(ms->GetLmax()), rank_J(rJ), rank_T(rT), parity(p) {}
...
ThreeBodyStorage::Permutation ThreeBodyStorage::SortOrbits(int a_in, int b_in, int c_in, int &a, int &b, int &c) const
{
    ...
    c = c_in;
    if (a > b)
        std::swap(a, b); // the a <= b <= c ordering matches with the 2body storage, which have if (q<p) continue;
    if (b > c)
        std::swap(b, c);
    if (a > b)
        std::swap(a, b);
...
{
    if (std::abs(int(ja - jb)) > Jab or int(ja + jb) < Jab)
        return 0;
...
int ThreeBodyStorage::PermutationPhase(ThreeBodyStorage::Permutation recoupling_case) const { return (recoupling_case == ABC or recoupling_case == BCA or recoupling_case == CAB) ? +1 : -1; }
...
std::vector<ThreeBodyStorage::Permutation> ThreeBodyStorage::UniquePermutations(size_t a, size_t b, size_t c) const
{
    ...
    if (a != b)
        unique_perms.push_back(BAC);
    if (a != c)
        unique_perms.push_back(CBA);
    if (b != c)
        unique_perms.push_back(ACB);
...
bool ThreeBodyStorage::IsKetInEMaxTruncations(size_t a, size_t b, size_t c) const
{
    ...
    if ((e_a > modelspace->GetEMax3Body()) || (e_b > modelspace->GetEMax3Body()) || (e_c > modelspace->GetEMax3Body()))
    {
        return false;
    }
...
size_t ThreeBodyStorage::GetKetIndex_withRecoupling(int Jab_in, int twoJ, size_t a_in, size_t b_in, size_t c_in, std::vector<size_t> &iket, std::vector<double> &recouple) const
{
    ...
    if ((a_in == b_in and (Jab_in % 2) > 0) or (a_in == b_in and (Jab_in > (modelspace->GetOrbit(a_in).j2 - 1))) or (a_in == b_in and a_in == c_in and (twoJ > (3 * oa.j2 - 3))) or (twoJ == (oa.j2 + ob.j2 + oc.j2) and (a == b or a == c or b == c)))
    {
        Jab_max = Jab_min - 1;
    }
...
std::vector<ThreeBodyStorage::ME_type> ThreeBodyStorage::GetME_pn_TwoOps(int Jab, int Jde, int twoJ, int a, int b, int c, int d, int e, int f, const ThreeBodyStorage &X, const ThreeBodyStorage &Y) const
{
    ...
    if (!IsKetValid(Jab, twoJ, a, b, c) || !IsKetValid(Jde, twoJ, d, e, f))
        return me_out;
